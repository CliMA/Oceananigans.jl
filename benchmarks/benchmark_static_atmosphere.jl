...
model = CompressibleModel(grid=grid, thermodynamic_variable=PrognosticE())
time_step!(model, Δt=1, Nt=2)  # warmup to compile

for i in 1:10
    @timeit timer "32×32×32 [CPU, Float64, Energy]" time_step!(model, Δt=1, Nt=1)
end
...
for i in 1:10
    @timeit timer "32×32×32 [CPU, Float64, Entropy]" time_step!(model, Δt=1, Nt=1)
end
