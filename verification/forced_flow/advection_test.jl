...
function run_convergence_test(κ, U, resolutions...)
    results = [run_test(Nx=Nx, CFL=0.01, U=U, κ=κ, width=0.1) for Nx in resolutions]
    return results
end
...
close("all")
fig, axs = subplots(nrows=2, figsize=(10, 6), sharex=true)
...
legend(loc="upper left", prop=Dict(:size=>6))
