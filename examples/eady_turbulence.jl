...
# his classic paper "Long waves and cyclone waves," Tellus (1949). The Eady problem is a
# simple, canonical model for the generation of mid-latitude atmospheric storms
# and the ocean eddies that permeate the world sea.
#
...
grid = RegularCartesianGrid(size=(64, 64, 16), x=(0, 2e5), y=(0, 2e5), z=(-4e3, 0))
...
uᵢ(x, y, z) = Ũ * (cos(4π / grid.Ly * y) / 10 + Ξ(z))
vᵢ(x, y, z) = Ũ * (sin(4π / grid.Lx * x) / 10 + Ξ(z))
bᵢ(x, y, z) = B̃ * Ξ(z)
...
anim = @animate for (i, iter) in enumerate(iterations)
    ...
    plot(ζ_xy, δ_xy, ζ_xz, δ_xz,
           size = (1000, 800),
           link = :x,
         layout = Plots.grid(2, 2, heights=[0.5, 0.5, 0.2, 0.2]),
          title = [@sprintf("ζ(t=%s) (s⁻¹)", prettytime(t)) @sprintf("δ(t=%s) (s⁻¹)", prettytime(t)) "" ""])

    iter == iterations[end] && close(file)
end

mp4(anim, "eady_turbulence.mp4", fps = 16) # hide
