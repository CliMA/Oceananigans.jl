...
for i=1:100
    ...
    pW = contour(xC, zC, W_slice; xlabel="x", ylabel="z", fill=true, levels=10, color=:balance, clims=(-0.1, 0.1))
    pρ = contour(xC, zC, ρ_slice; xlabel="x", ylabel="z", fill=true, levels=10, color=:balance, clims=(-1e-5, 1e-5))
    pΘ = contour(xC, zC, Θ_slice; xlabel="x", ylabel="z", fill=true, levels=10, color=:thermal, clims=(0, 0.01))
    
    display(plot(pU, pW, pρ, pΘ, title=["U" "W" "rho_prime" "Theta_m_prime"], show=true))
end
