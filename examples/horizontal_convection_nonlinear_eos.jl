...
# ```julia
# using Pkg
# pkg"add Oceananigans, SeawaterPolynomials, JLD2, Plots"
# ```
...
using Oceananigans
using SeawaterPolynomials: RoquetEquationOfState, ρ′
using Printf
...
#
# ```math
# T(x, z=0, t) = - T_* \cos (2π x / L_x) \, .
# ```

const T★ = 5.0
const background_temperature = 10 # ᵒC

@inline Tˢ(x, y, t) = - T★ * cos(π * x) + background_temperature

top_bc = ValueBoundaryCondition(Tˢ)
T_bcs = FieldBoundaryConditions(top = top_bc)

ρ₀ = 1000

roquet_eos = RoquetEquationOfState(:Cabbeling, reference_density=ρ₀)
buoyancy = SeawaterBuoyancy(equation_of_state=roquet_eos, constant_salinity=0)

b₋ = buoyancy.gravitational_acceleration * ρ′(background_temperature + T★, 0, 0, roquet_eos) / ρ₀
b₊ = buoyancy.gravitational_acceleration * ρ′(background_temperature - T★, 0, 0, roquet_eos) / ρ₀
...
Ra = 1e16   # The Rayleigh number

ν = sqrt(Pr * Δb * Lx^3 / Ra)  # Laplacian viscosity
...
max_Δt = t★/5
wizard = TimeStepWizard(cfl=0.8, Δt=t★/20, max_change=1.2, max_Δt=max_Δt)
...
b = BuoyancyField(model)   # unpack buoyancy `Field`
...
anim = @animate for i in 1:50:length(times)
...
mp4(anim, "horizontal_convection_nonlin_eos.mp4", fps = 16) # hide
