...
# Here we could also add MPI examples!!!
# TODO: write an MPI example on the GPU and run it here with, for example:
# example = mpi_gpu_example_script
# example_filepath = joinpath(EXAMPLES_DIR, example)
# withenv("JULIA_DEBUG" => "Literate") do
#     start_time = time_ns()
#     Literate.markdown(example_filepath, OUTPUT_DIR;
...
# This is done only on rank 0 to avoid conflicts with the file system
