...
\end{align}
```
Incorporating the definition of ``\omega`` in \eqref{div1} we get
```math
\begin{align}
...
It is useful to think about mass conservation \eqref{massconservationr} in the discrete form in a finite volume staggered C-grid framework.
...
Indices `i`, `j`, `k` correspond to the `x`, `y`, and the vertical directions respectively.
```math
\begin{equation}
