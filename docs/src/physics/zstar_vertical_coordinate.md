...
```
where $\boldsymbol{u}_h = (u, v)$ is the horizontal velocity, $\zeta = \partial_x v - \partial_y u$ is the vertical vorticity and $K = (u^2 + v^2)/2$ is the horizontal kinetic energy.
