...
function create_nccl_comm_from_mpi(mpi_subcomm)
    ...
    try
        ...
        # UniqueID.internal is NTuple{128, Cchar}, but the MPI used on CI doesn't seem to
        # like broadcasting a `Vector{Cchar}`, so we use a `Vector{UInt8}` instead.
        ...
        nccl_internal = ntuple(i -> reinterpret(Cchar, id_bytes[i]), Val(128))
