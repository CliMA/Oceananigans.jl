...
# Extract `TimeSeriesInterpolation` directly so its clock is synchronized with
# the model clock in addition to updating the underlying `FieldTimeSeries`.
extract_field_time_series(f::TimeSeriesInterpolation) = f
