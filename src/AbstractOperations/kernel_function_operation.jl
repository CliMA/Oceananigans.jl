struct KernelFunctionOperation{LX, LY, LZ, P, A, G, T, K, D} <: AbstractOperation{LX, LY, LZ, A, G, T}
    ...
    """
        KernelFunctionOperation{LX, LY, LZ}(kernel_function, grid;
                                            architecture=nothing, computed_dependencies=(), parameters=nothing)
    ...
    ```
    """
    function KernelFunctionOperation{LX, LY, LZ}(kernel_function::K, architecture::A, grid::G;
                                                 computed_dependencies::D=(), parameters::P=nothing) where {LX, LY, LZ, K, G, A, D, P}
        T = eltype(grid)
        return new{LX, LY, LZ, P, A, G, T, K, D}(kernel_function, grid, architecture, computed_dependencies, parameters)
    end
end

@inline Base.getindex(κ::KernelFunctionOperation, i, j, k) = κ.kernel_function(i, j, k, grid, κ.computed_dependencies..., κ.parameters)
@inline Base.getindex(κ::KernelFunctionOperation{LX, LY, LZ, <:Nothing}, i, j, k) where {LX, LY, LZ} = κ.kernel_function(i, j, k, grid, κ.computed_dependencies...)
...
Adapt.adapt_structure(to, κ::KernelFunctionOperation{LX, LY, LZ}) where {LX, LY, LZ}
    KernelFunctionOperation{X, Y, Z}(Adapt.adapt(to, κ.kernel_function),
