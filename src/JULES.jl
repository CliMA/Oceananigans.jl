...
import Oceananigans.BoundaryConditions: NFBC,
  ...
  _fill_west_halo!(c, ::NFBC, H, N) = @views @. c.parent[1:1+H, :, :] = 0
 _fill_south_halo!(c, ::NFBC, H, N) = @views @. c.parent[:, 1:1+H, :] = 0
_fill_bottom_halo!(c, ::NFBC, H, N) = @views @. c.parent[:, :, 1:1+H] = 0

 _fill_east_halo!(c, ::NFBC, H, N) = @views @. c.parent[N+H+1:N+2H, :, :] = 0
_fill_north_halo!(c, ::NFBC, H, N) = @views @. c.parent[:, N+H+1:N+2H, :] = 0
  _fill_top_halo!(c, ::NFBC, H, N) = @views @. c.parent[:, :, N+H+1:N+2H] = 0
...
for (x, side) in zip(coords, sides)
    ...
    N = Symbol(:N, x)
    @eval begin
        $outername(c, bc::NFBC, arch, grid, args...) =
