...
function time_average_outputs(schedule::AveragedTimeInterval, outputs::Dict, model, field_slicer)
    averaged_outputs = Dict(name => WindowedTimeAverage(output, model; schedule=schedule, field_slicer=field_slicer)
...
function time_average_outputs(schedule::AveragedTimeInterval, outputs::NamedTuple, model, field_slicer)
    ...
    output_values = Tuple(WindowedTimeAverage(outputs[name], model; schedule=schedule, field_slicer=field_slicer)
