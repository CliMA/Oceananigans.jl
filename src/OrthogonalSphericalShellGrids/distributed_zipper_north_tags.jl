...
function north_recv_tag(arch, ::MPITripolarGridOfSomeKind, field_id, location)
    field_tag   = string(field_id, pad=ID_DIGITS)
    ...
    return parse(Int, field_tag * loc_digit * side_digit)
end

function north_send_tag(arch, ::MPITripolarGridOfSomeKind, field_id, location)
    field_tag   = string(field_id, pad=ID_DIGITS)
    loc_digit  = string(loc_id(location...), pad=ID_DIGITS)
    last_rank  = arch.local_index[2] == ranks(arch)[2]
...
function northwest_recv_tag(arch, ::MPITripolarGridOfSomeKind, field_id, location)
    field_tag   = string(field_id, pad=ID_DIGITS)
    loc_digit  = string(loc_id(location...), pad=ID_DIGITS)
    last_rank  = arch.local_index[2] == ranks(arch)[2]
...
function northwest_send_tag(arch, ::MPITripolarGridOfSomeKind, field_id, location)
    field_tag   = string(field_id, pad=ID_DIGITS)
    loc_digit  = string(loc_id(location...), pad=ID_DIGITS)
    last_rank  = arch.local_index[2] == ranks(arch)[2]
...
function northeast_recv_tag(arch, ::MPITripolarGridOfSomeKind, field_id, location)
    field_tag   = string(field_id, pad=ID_DIGITS)
    loc_digit  = string(loc_id(location...), pad=ID_DIGITS)
    last_rank  = arch.local_index[2] == ranks(arch)[2]
...
function northeast_send_tag(arch, ::MPITripolarGridOfSomeKind, field_id, location)
    field_tag   = string(field_id, pad=ID_DIGITS)
    loc_digit  = string(loc_id(location...), pad=ID_DIGITS)
    last_rank  = arch.local_index[2] == ranks(arch)[2]
