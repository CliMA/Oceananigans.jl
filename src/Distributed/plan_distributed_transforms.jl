...
function plan_distributed_transforms(global_grid::RegRectilinearGrid, storage::ParallelFields, planner_flag)
    ...
    forward_plan_z  = plan_forward_transform(interior(storage.zfield),  topo[3](), [3], planner_flag)
    backward_plan_x = plan_backward_transform(interior(storage.xfield), topo[1](), [1], planner_flag)
    backward_plan_z = plan_backward_transform(interior(storage.zfield), topo[3](), [3], planner_flag)
    ...
    if arch isa GPU
        ...
        forward_plan_y  = plan_forward_transform(interior(storage.yfield),  topo[2](), [2], planner_flag) 
        backward_plan_y = plan_backward_transform(interior(storage.yfield), topo[2](), [2], planner_flag) 
    end
