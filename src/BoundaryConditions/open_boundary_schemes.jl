...
#####
##### Distributed GPU-compatible Flather fills (match kernel call signature with args...)
#####

# GPU kernels call with args... so we need to accept that and unpack it
@inline function _fill_west_halo!(j, k, grid, c, bc::FNBCBC, loc, args...)
    # Unpack args: first is buffers (ignored), second is clock, third is model_fields
    clock = args[2]
    model_fields = args[3]
...
@inline function _fill_east_halo!(j, k, grid, c, bc::FNBCBC, loc, args...)
    clock = args[2]
    model_fields = args[3]
...
@inline function _fill_south_halo!(i, k, grid, c, bc::FNBCBC, loc, args...)
    clock = args[2]
    model_fields = args[3]
...
@inline function _fill_north_halo!(i, k, grid, c, bc::FNBCBC, loc, args...)
    clock = args[2]
    model_fields = args[3]
