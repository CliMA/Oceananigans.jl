...
"""
apply_x_bcs!(Gc, c, arch, dep, args...) = launch!(arch, c.grid, :yz, _apply_x_bcs!, Gc, c.grid, 
...
the source term `Gc` at the left and right.
"""
apply_y_bcs!(Gc, c, arch, dep, args...) = launch!(arch, c.grid, :xz, _apply_y_bcs!, Gc, c.grid, 
...
"""
apply_z_bcs!(Gc, c, arch, dep, args...) = launch!(arch, c.grid, :xy, _apply_z_bcs!, Gc, c.grid, 
...
@inline apply_x_east_bc!(  Gc::AbstractField, args...) = nothing
@inline apply_x_west_bc!(  Gc::AbstractField, args...) = nothing
@inline apply_y_north_bc!( Gc::AbstractField, args...) = nothing
@inline apply_y_south_bc!( Gc::AbstractField, args...) = nothing
@inline apply_z_top_bc!(   Gc::AbstractField, args...) = nothing
@inline apply_z_bottom_bc!(Gc::AbstractField, args...) = nothing

# Shortcuts for 'zero' flux boundary conditions.
@inline apply_x_east_bc!(  Gc::AbstractField, ::ZFBC, args...) = nothing
@inline apply_x_west_bc!(  Gc::AbstractField, ::ZFBC, args...) = nothing
@inline apply_y_north_bc!( Gc::AbstractField, ::ZFBC, args...) = nothing
@inline apply_y_south_bc!( Gc::AbstractField, ::ZFBC, args...) = nothing
@inline apply_z_top_bc!(   Gc::AbstractField, ::ZFBC, args...) = nothing
@inline apply_z_bottom_bc!(Gc::AbstractField, ::ZFBC, args...) = nothing
...
"""
@inline apply_x_west_bc!(Gc::AbstractField, west_flux::BC{<:Flux}, j, k, grid, args...) =
    @inbounds Gc[1, j, k] += getbc(west_flux,   j, k, grid, args...) /  Δx(1, j, k, grid)

@inline apply_y_south_bc!(Gc::AbstractField, south_flux::BC{<:Flux}, i, k, grid, args...) =
    @inbounds Gc[i, 1, k] += getbc(south_flux,  i, k, grid, args...) /  Δy(i, 1, k, grid)

@inline apply_z_bottom_bc!(Gc::AbstractField{X, Y, Z}, bottom_flux::BC{<:Flux}, i, j, grid, args...) where {X, Y, Z} =
    @inbounds Gc[i, j, 1] += getbc(bottom_flux, i, j, grid, args...) * Az(i, j, 1, grid, X, Y, flip(Z)) / volume(i, j, 1, grid, X, Y, Z)
...
"""
@inline apply_x_east_bc!(Gc::AbstractField, east_flux::BC{<:Flux}, j, k, grid, args...) =
    @inbounds Gc[grid.Nx, j, k] -= getbc(east_flux,  j, k, grid, args...) /  Δx(grid.Nx, j, k, grid)

@inline apply_y_north_bc!(Gc::AbstractField, north_flux::BC{<:Flux}, i, k, grid, args...) =
    @inbounds Gc[i, grid.Ny, k] -= getbc(north_flux, i, k, grid, args...) /  Δy(i, grid.Ny, k, grid)

@inline apply_z_top_bc!(Gc::AbstractField{X, Y, Z}, top_flux::BC{<:Flux}, i, j, grid, args...) where {X, Y, Z} =
    @inbounds Gc[i, j, grid.Nz] -= getbc(top_flux, i, j, grid, args...) * Az(i, j, grid.Nz+1, grid, X, Y, flip(Z)) / volume(i, j, grid.Nz, grid, X, Y, Z)
