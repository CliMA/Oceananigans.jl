...
"""
    validate_implicit_explicit_flux_locations(bcs)

Error if an [`ImplicitExplicitFluxBoundaryCondition`](@ref) is set on any boundary other than
`top`/`bottom`. The implicit part is embedded in the *vertical* solver only, so a horizontal or
immersed placement would silently drop it.
"""
