...
@kernel function set_east_west_u_velocity!(u, i_boundary, bc, grid, loc, args...)
    j, k = @index(Global, NTuple)
    @inbounds u[i_boundary, j, k] = getbc(bc, j, k, grid, args...)
end

@kernel function set_north_south_v_velocity!(v, j_boundary, bc, grid, loc, args...)
    i, k = @index(Global, NTuple)
    @inbounds v[i, j_boundary, k] = getbc(bc, i, k, grid, args...)
end

@kernel function set_top_bottom_w_velocity!(w, k_boundary, bc, grid, loc, args...)
