...
function fill_halo_regions!(c::MaybeTupledData, boundary_conditions, indices, loc, grid, args...; async = false, barrier = device_event(architecture(grid)), kwargs...)
...
function fill_halo_event!(task, halo_tuple, c, indices, loc, arch, events, dependency, grid, args...; kwargs...)
    ...
    barrier = ifelse(isempty(events), dependency, events[end])
