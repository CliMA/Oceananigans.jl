...
function BuoyancyModel(; model, gravitational_unit_vector=(0, 0, 1))
    ...
    return BuoyancyModel(model, gravitational_unit_vector)
end

@inline required_tracers(bm::BuoyancyModel) = required_tracers(bm.model)

@inline get_temperature_and_salinity(bm::BuoyancyModel, C) = get_temperature_and_salinity(bm.model, C)
