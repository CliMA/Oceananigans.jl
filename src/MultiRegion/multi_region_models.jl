...
import Oceananigans.Advection: WENO5, compute_stretched_weno_coefficients
...
@inline @inbounds getregion(w5::WENO5, i)                    = WENO5(Tuple(getregion(getproperty(w5, propertynames(w5)[idx]), i) for idx in 1:length(propertynames(w5)))...)
...
compute_stretched_weno_coefficients(grid::MultiRegionGrid, stretched_smoothness) = 
      construct_regionally(compute_stretched_weno_coefficients, grid, stretched_smoothness)
