...
function fill_halo_regions!(grid::ConformalCubedSphereGrid{FT, TX, TY, TZ}) where {FT, TX, TY, TZ}
    ...
    λᶜᶜᵃ  = Field((Center, Center, Nothing), grid, indices = (:, :, 1:1))
    φᶜᶜᵃ  = Field((Center, Center, Nothing), grid, indices = (:, :, 1:1))
    Azᶜᶜᵃ = Field((Center, Center, Nothing), grid, indices = (:, :, 1:1))
    λᶠᶠᵃ  = Field((Face,   Face,   Nothing), grid, indices = (:, :, 1:1))
    φᶠᶠᵃ  = Field((Face,   Face,   Nothing), grid, indices = (:, :, 1:1))
    Azᶠᶠᵃ = Field((Face,   Face,   Nothing), grid, indices = (:, :, 1:1))
    ...
    Δxᶜᶜᵃ = Field((Center, Center, Nothing), grid, indices = (:, :, 1:1))
    Δxᶠᶜᵃ = Field((Face,   Center, Nothing), grid, indices = (:, :, 1:1))
    Δyᶠᶜᵃ = Field((Face,   Center, Nothing), grid, indices = (:, :, 1:1))
    λᶠᶜᵃ  = Field((Face,   Center, Nothing), grid, indices = (:, :, 1:1))
    φᶠᶜᵃ  = Field((Face,   Center, Nothing), grid, indices = (:, :, 1:1))
    Azᶠᶜᵃ = Field((Face,   Center, Nothing), grid, indices = (:, :, 1:1))
    Δxᶠᶠᵃ = Field((Face,   Face,   Nothing), grid, indices = (:, :, 1:1))

    fields₁ = ( Δxᶜᶜᵃ,   Δxᶠᶜᵃ,   Δyᶠᶜᵃ,   λᶠᶜᵃ,    φᶠᶜᵃ,    Azᶠᶜᵃ ,  Δxᶠᶠᵃ)
    names₁  = (:Δxᶜᶜᵃ,  :Δxᶠᶜᵃ,  :Δyᶠᶜᵃ,  :λᶠᶜᵃ,   :φᶠᶜᵃ,   :Azᶠᶜᵃ , :Δxᶠᶠᵃ)

    Δyᶜᶜᵃ = Field((Center, Center, Nothing), grid, indices = (:, :, 1:1))
    Δyᶜᶠᵃ = Field((Center, Face,   Nothing), grid, indices = (:, :, 1:1))
    Δxᶜᶠᵃ = Field((Center, Face,   Nothing), grid, indices = (:, :, 1:1))
    λᶜᶠᵃ  = Field((Center, Face,   Nothing), grid, indices = (:, :, 1:1))
    φᶜᶠᵃ  = Field((Center, Face,   Nothing), grid, indices = (:, :, 1:1))
    Azᶜᶠᵃ = Field((Center, Face,   Nothing), grid, indices = (:, :, 1:1))
    Δyᶠᶠᵃ = Field((Face,   Face,   Nothing), grid, indices = (:, :, 1:1))
