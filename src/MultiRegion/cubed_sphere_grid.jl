...
function ConformalCubedSphereGrid(arch::AbstractArchitecture=CPU(),
    ...
    connectivity = CubedSphereConnectivity(partition)
    ...
    new_region_grids = MultiRegionObject(new_region_grids.regional_objects)
...
function ConformalCubedSphereGrid(filepath::AbstractString,
    ...
    partition = CubedSpherePartition(R = 1)
    region_Nz = MultiRegionObject(Tuple(repeat([Nz], length(partition))))
...
function SplitExplicitFreeSurfaces.maybe_extend_halos(TX, TY, grid::ConformalCubedSphereGridOfSomeKind, substepping::FixedSubstepNumber, stages)
