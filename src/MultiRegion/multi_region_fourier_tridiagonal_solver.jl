...
# PressureSolver(arch, grid::HRegMultiRegionGrid, planner_flag=FFTW.PATIENT) = MultiRegionFFTBasedPMultiRegionFourierTridiagonalSolveroissonSolver(grid, planner_flag)

function MultiRegionFourierTridiagonalnSolver(grid::MultiRegionGrid, planner_flag=FFTW.PATIENT)
    ...
    return MultiRegionFFTBasedPoissonSolver(grid, tgrid, btsolver, storage, buffer, operations, source_term)
end
...
function solve!(ϕ, solver::MultiRegionFFTBasedPoissonSolver, m=0)
    ...
    @apply_regionally begin
        apply_multi_operation(operations.forward[1], source_term.initial, buffer.initial)
    end
    ...
    @apply_regionally begin
        solve!(storage.transposed, btsolver, source_term.transposed)
    end
    ...
    @apply_regionally begin
        apply_multi_operation(operations.backward[3], storage.initial, buffer.initial)
