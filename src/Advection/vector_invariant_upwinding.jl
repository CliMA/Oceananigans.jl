...
struct OnlySelfUpwinding{A, U, V, U2, V2} <: AbstractUpwindingTreatmen
...
struct CrossAndSelfUpwinding{A, D, U, V} <: AbstractUpwindingTreatmen
    cross_scheme       :: A # advection scheme for cross-reconstructed terms in the kinetic energy gradien
...
struct VelocityUpwinding{A} <: AbstractUpwindingTreatmen
    cross_scheme    :: A # advection scheme for cross-reconstructed terms (in both divergence flux and KE gradient)
end

# `cross_scheme` is used only for `_symmetric_interpolate`s (i.e., only where we do not allow upwinding of the tangetial term)
# Therefore, for `@show` purposes it is better to convert an upwind scheme to it's centered counterpar
