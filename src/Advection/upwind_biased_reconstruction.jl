...
for buffer in advection_buffers
    @eval begin
        @inline inner_biased_interpolate_xᶠᵃᵃ(i, j, k, grid, ::UX{$buffer, FT}, left_bias, ψ, idx, loc, args...) where FT = 
            @inbounds ifelse(left_bias, $(calc_reconstruction_stencil(buffer, :left,  :x, false)), 
                                        $(calc_reconstruction_stencil(buffer, :right, :x, false)))

        @inline inner_biased_interpolate_xᶠᵃᵃ(i, j, k, grid, ::UX{$buffer, FT}, left_bias, ψ::Function, idx, loc, args...) where FT = 
            @inbounds ifelse(left_bias, $(calc_reconstruction_stencil(buffer, :left,  :x, true)), 
                                        $(calc_reconstruction_stencil(buffer, :right, :x, true)))
    
        @inline inner_biased_interpolate_yᵃᶠᵃ(i, j, k, grid, ::UY{$buffer, FT}, left_bias, ψ, idx, loc, args...) where FT = 
            @inbounds ifelse(left_bias, $(calc_reconstruction_stencil(buffer, :left,  :y, false)), 
                                        $(calc_reconstruction_stencil(buffer, :right, :y, false)))
                                                 
        @inline inner_biased_interpolate_yᵃᶠᵃ(i, j, k, grid, ::UY{$buffer, FT}, left_bias, ψ::Function, idx, loc, args...) where FT = 
            @inbounds ifelse(left_bias, $(calc_reconstruction_stencil(buffer, :left,  :y, true)), 
                                        $(calc_reconstruction_stencil(buffer, :right, :y, true)))
    
        @inline inner_biased_interpolate_zᵃᵃᶠ(i, j, k, grid, ::UZ{$buffer, FT}, left_bias, ψ, idx, loc, args...) where FT = 
            @inbounds ifelse(left_bias, $(calc_reconstruction_stencil(buffer, :left,  :z, false)), 
                                        $(calc_reconstruction_stencil(buffer, :right, :z, false)))

        @inline inner_biased_interpolate_zᵃᵃᶠ(i, j, k, grid, ::UZ{$buffer, FT}, left_bias, ψ::Function, idx, loc, args...) where FT = 
            @inbounds ifelse(left_bias, $(calc_reconstruction_stencil(buffer, :left,  :z, true)), 
