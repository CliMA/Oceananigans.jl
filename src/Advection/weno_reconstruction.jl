...
struct WENO{N, FT, FTS, PP, SI} <: AbstractUpwindBiasedAdvectionScheme{N, FT}
    ...
    function WENO{N, FT, FTS}(bounds::PP, 
                              advecting_velocity_scheme :: SI) where {N, FT, FTS, PP, SI}

        return new{N, FT, FTS, PP, SI}(bounds, advecting_velocity_scheme)
    end
end
...
function WENO(FT::DataType=Oceananigans.defaults.FloatType, FTS::DataType=Float32;
    ...
    return WENO{N, FT, FTS}(bounds, advecting_velocity_scheme)
end
