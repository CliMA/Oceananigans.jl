for side in [:left, :right], (dir, val) in zip([:xᶠᵃᵃ, :yᵃᶠᵃ, :zᵃᵃᶠ], [1, 2, 3])
    ...
    @eval begin
        @inline function $biased_interpolate(i, j, k, grid, 
            ...
            glob += add_global_smoothness(glob, β, Val(N), Val(4))
            sol1 += ψ̅ * C
            wei1 += C
            sol2 += ψ̅ * α  
            wei2 += α
        ...
        @inline function $biased_interpolate(i, j, k, grid, 
            ...
            α  = @fastmath C / (βU + FT(ε))^2
            ψ̅  = $biased_p(scheme, Val(4), ψs, Nothing, Val($val), idx, loc) 
            glob += add_global_smoothness(glob, βU, Val(N), Val(4))
            sol1 += ψ̅ * C
            wei1 += C
            sol2 += ψ̅ * α  
            wei2 += α
        ...
        @inline function $biased_interpolate(i, j, k, grid, 
            ...
            α  = @fastmath C / (βϕ + FT(ε))^2
            ψ̅  = $biased_p(scheme, Val(1), ψs, Nothing, Val($val), idx, loc) 
            glob += add_global_smoothness(glob, βϕ, Val(N), Val(1))
            sol1 += ψ̅ * C
            wei1 += C
            sol2 += ψ̅ * α  
            wei2 += α
            ...
            α  = @fastmath C / (βϕ + FT(ε))^2
            ψ̅  = $biased_p(scheme, Val(2), ψs, Nothing, Val($val), idx, loc) 
            glob += add_global_smoothness(glob, βϕ, Val(N), Val(2))
            sol1 += ψ̅ * C
            wei1 += C
            sol2 += ψ̅ * α  
            wei2 += α
            ...
            α  = @fastmath C / (βϕ + FT(ε))^2
            ψ̅  = $biased_p(scheme, Val(3), ψs, Nothing, Val($val), idx, loc) 
            glob += add_global_smoothness(glob, βϕ, Val(N), Val(3))
            sol1 += ψ̅ * C
            wei1 += C
            sol2 += ψ̅ * α  
            wei2 += α
            ...
            α  = @fastmath C / (βϕ + FT(ε))^2
            ψ̅  = $biased_p(scheme, Val(4), ψs, Nothing, Val($val), idx, loc) 
            glob += add_global_smoothness(glob, βϕ, Val(N), Val(4))
            sol1 += ψ̅ * C
            wei1 += C
            sol2 += ψ̅ * α  
            wei2 += α
