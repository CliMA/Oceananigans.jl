...
@inline reconstruction_tuple(::Val{S}, ::Val{B}) where {S, B} = stencil_coefficients(50, S, collect(1:100), collect(1:100); order = B)

# uniform coefficients are independent on direction and location
for (coefficients, coefficient_tuple) in zip((β_coefficients, p_coefficients), (smoothness_tuple, reconstruction_tuple))
    @eval begin
...
for buffer in [2, 3, 4, 5, 6]
    for stencil in collect(0:1:buffer-1)
            ...
            @inline coeff_p(scheme::WENO{$buffer}, order, bias, ::Val{$stencil}, ::Type{Nothing}, dir, i, loc) = p_coefficients(order, Val(buffer), Val(stencil))

            # stretched coefficients are retrieved from precalculated coefficients
            @inline coeff_p(scheme::WENO{$buffer}, order, bias, ::Val{$stencil}, T, dir, i, loc) = 
