...
for buffer in [2, 3, 4, 5, 6]
    for stencil in collect(0:1:buffer-1)
        ...
        @eval begin
            ...
            @inline coeff_p(scheme::WENO{$buffer, FT}, ::Val{$stencil}, ::Type{Nothing}, args...) where FT = @inbounds FT.($(stencil_coefficients(50, stencil  , collect(1:100), collect(1:100); order = buffer)))
            
            # stretched coefficients are retrieved from precalculated coefficients (remember to revert them accordingly!!!)
            @inline coeff_p(scheme::WENO{$buffer}, ::Val{$stencil}, T, dir, i, loc) = @inbounds retrieve_coeff(scheme, $stencil, dir, i, loc)
        end
    
        # left biased and right biased reconstruction value for each stencil
        @eval begin
            @inline biased_p(scheme::WENO{$buffer}, ::Val{$stencil}, ψ, T, dir, i, loc) = @inbounds sum(coeff_p(scheme, Val($stencil), T, dir, i, loc) .* ψ)
        end
    end
end
...
@inline function upwind_stencil_sum(scheme::WENO{N, FT}, ψ, w, T, side_index, idx, loc) where N
    t = FT(0)
    @unroll for i in 1:N
