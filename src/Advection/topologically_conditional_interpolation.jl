    ...
    for loc in (:ᶜ, :ᶠ)
        ...
        if ξ == :x
            @eval begin
                ...
                @inline $alt_interp(i, j, k, grid::AUGX, scheme::HOADV, ::Val{D}, f::Function, args...) where D =
                    ifelse($outside_buffer(i, grid.Nx, scheme, Val(D)),
                            $interp(i, j, k, grid, scheme, Val(D), f, args...),
                            $alt_interp(i, j, k, grid, scheme.buffer_scheme, Val(D), f, args...))
            end
        elseif ξ == :y
            @eval begin
                ...
                @inline $alt_interp(i, j, k, grid::AUGY, scheme::HOADV, ::Val{D}, f::Function, args...) where D =
                    ifelse($outside_buffer(j, grid.Ny, scheme, Val(D)),
                            $interp(i, j, k, grid, scheme, Val(D), f, args...),
                            $alt_interp(i, j, k, grid, scheme.buffer_scheme, Val(D), f, args...))
            end
        elseif ξ == :z
            @eval begin
                ...
                @inline $alt_interp(i, j, k, grid::AUGZ, scheme::HOADV, ::Val{D}, f::Function, args...) where D =
                    ifelse($outside_buffer(k, grid.Nz, scheme, Val(D)),
                            $interp(i, j, k, grid, scheme, Val(D), f, args...),
                            $alt_interp(i, j, k, grid, scheme.buffer_scheme, Val(D), f, args...))
            end
