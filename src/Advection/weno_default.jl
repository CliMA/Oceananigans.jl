...
for side in [:left, :right], (dir, val, CT) in zip([:xᶠᵃᵃ, :yᵃᶠᵃ, :zᵃᵃᶠ], [1, 2, 3], [:XT, :YT, :ZT])
    ...
    @eval begin
        ...
        @inline function $biased_interpolate(i, j, k, grid, 
            ...
            β, ψ̅, C, α = $weno_interpolant(ψs[2:3], 1, grid, scheme, $val, ψ, idx, loc, args...)
            τ  = β
            ψ̂₁ = ψ̅ * C
            w₁ = C
            ψ̂₂ = ψ̅ * α  
            ...
            # Stencil S₁
            β, ψ̅, C, α = $weno_interpolant(ψs[1:2], 2, grid, scheme, $val, ψ, idx, loc, args...)
        ...
        @inline function $biased_interpolate(i, j, k, grid, 
            ...
            # Stencil S₁
            β, ψ̅, C, α = $weno_interpolant(ψs[1:5], 4, scheme, $val, idx, loc)
        ...
        @inline function $biased_interpolate(i, j, k, grid, 
            ...
            # Stencil S₁
            β, ψ̅, C, α = $weno_interpolant(ψs[2:7], 4, scheme, $val, idx, loc)
            ...
            # Stencil S₁
            β, ψ̅, C, α = $weno_interpolant(ψs[1:6], 4, scheme, $val, idx, loc)
