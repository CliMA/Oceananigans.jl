...
@inline function upwinded_Ax_uᶜᶜᶜ(i, j, k, grid, scheme, u) 
    ...
    return _biased_interpolate_xᶜᵃᵃ(i, j, k, grid, scheme, scheme.divergence_scheme, bias(û), Ax_qᶠᶜᶜ, u)
end

@inline function upwinded_Ay_vᶜᶜᶜ(i, j, k, grid, scheme, v) 
    v̂ = ℑyᵃᶜᵃ(i, j, k, grid, v)
    return _biased_interpolate_yᵃᶜᵃ(i, j, k, grid, scheme, scheme.divergence_scheme, bias(v̂), Ay_qᶜᶠᶜ, v)
end
...
@inline function upwinded_u²ᶜᶜᶜ(i, j, k, grid, scheme, u) 
    û = ℑxᶜᵃᵃ(i, j, k, grid, u)
    return _biased_interpolate_xᶜᵃᵃ(i, j, k, grid, scheme, scheme.kinetic_energy_gradient_scheme, bias(û), half_ϕ², u)
end

@inline function upwinded_v²ᶜᶜᶜ(i, j, k, grid, scheme, v) 
    v̂ = ℑyᵃᶜᵃ(i, j, k, grid, v)
    return _biased_interpolate_yᵃᶜᵃ(i, j, k, grid, scheme, scheme.kinetic_energy_gradient_scheme, bias(v̂), half_ϕ², v)
end
