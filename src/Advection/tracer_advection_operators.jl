...
@inline function horizontal_div_Uc(i, j, k, grid, advection, U, c)
    return 1/Vᶜᶜᶜ(i, j, k, grid) * (δxᶜᵃᵃ(i, j, k, grid, _advective_tracer_flux_x, advection, U.u, c) +
                                    δyᵃᶜᵃ(i, j, k, grid, _advective_tracer_flux_y, advection, U.v, c))
end
...
# Fallbacks for zero velocities, zero tracer and `nothing` advection
@inline horizontal_div_Uc(i, j, k, grid, advection, ::ZeroU, c) = zero(grid)
@inline horizontal_div_Uc(i, j, k, grid, advection, U, ::ZeroField) = zero(grid)
@inline horizontal_div_Uc(i, j, k, grid, advection, ::ZeroU, ::ZeroField) = zero(grid)

@inline horizontal_div_Uc(i, j, k, grid, ::Nothing, U, c) = zero(grid)
@inline horizontal_div_Uc(i, j, k, grid, ::Nothing, ::ZeroU, c) = zero(grid)
@inline horizontal_div_Uc(i, j, k, grid, ::Nothing, U, ::ZeroField) = zero(grid)
@inline horizontal_div_Uc(i, j, k, grid, ::Nothing, ::ZeroU, ::ZeroField) = zero(grid)
