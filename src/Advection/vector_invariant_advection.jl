...
function Base.summary(a::WENOVectorInvariant{N}) where N
    ...
    WCT = weight_computation_type(a.vorticity_scheme)
    return string("WENOVectorInvariant{$N, $FT, $WCT}(vorticity_order=$vorticity_order, vertical_order=$vertical_order)")
end
...
for b in advection_buffers, FT in fully_supported_float_types
    @eval begin
        @inline symmetric_interpolate_xᶠᵃᵃ(i, j, k, grid, s::Centered{$b, $FT}, r::Int, f::Callable, ::AS, args...) = symmetric_interpolate_xᶠᵃᵃ(i, j, k, grid, s, r, f, args...)
