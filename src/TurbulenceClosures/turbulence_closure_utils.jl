...
@inline calc_nonlinear_νᶜᶜᶜ(i, j, k, grid, closure, buoyancy, U, C) = zero(grid)
@inline calc_nonlinear_κᶜᶜᶜ(i, j, k, grid, closure, c, tracer_index, U) = zero(grid)
...
@kernel function calculate_nonlinear_tracer_diffusivity!(κₑ, grid, closure, c, tracer_index, U)
    i, j, k = @index(Global, NTuple)
    @inbounds κₑ[i, j, k] = calc_nonlinear_κᶜᶜᶜ(i, j, k, grid, closure, c, tracer_index, U)
end
