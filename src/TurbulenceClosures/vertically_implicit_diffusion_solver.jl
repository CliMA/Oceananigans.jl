...
@inline ivd_diffusivity(i, j, k, grid, ::F, ::C, ::C, clo, K, ::Nothing, args...) = ifelse(inactive_node(i, j, k, grid, f, c, f), zero(grid), νzᶠᶜᶠ(i, j, k, grid, clo, K, args...))
@inline ivd_diffusivity(i, j, k, grid, ::C, ::F, ::C, clo, K, ::Nothing, args...) = ifelse(inactive_node(i, j, k, grid, c, f, f), zero(grid), νzᶜᶠᶠ(i, j, k, grid, clo, K, args...))
...
# Tracer diffusivity
@inline ivd_diffusivity(i, j, k, grid, ::C, ::C, ::C, args...) = ifelse(inactive_node(i, j, k, grid, c, c, c), zero(grid), κzᶜᶜᶠ(i, j, k, grid, args...))
...
@inline ivd_diagonal(i, j, k, grid, closure, K, id, ℓx, ℓy, ℓz, Δt, args...) =
    one(grid) - Δt * implicit_linear_coefficient(i, j, k,   grid, closure, K, id, ℓx, ℓy, ℓz, Δt, args...) -
                              ivd_upper_diagonal(i, j, k,   grid, closure, K, id, ℓx, ℓy, ℓz, Δt, args...) -
                              ivd_lower_diagonal(i, j, k-1, grid, closure, K, id, ℓx, ℓy, ℓz, Δt, args...)
