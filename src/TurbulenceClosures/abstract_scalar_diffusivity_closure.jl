...
@inline κᶠᶠᶜ(i, j, k, grid, loc, κ::Number, args...) = κ
...
@inline κᶠᶠᶜ(i, j, k, grid, ::Lᶜᶜᶜ, κ::AbstractArray, args...) = ℑxyᶠᶠᵃ(i, j, k, grid, κ)
...
@inline κᶠᶜᶠ(i, j, k, grid, loc, κ::F, clock, args...) where F<:Function = κ(node(i, j, k, grid, c, f, f)..., clock.time)
@inline κᶜᶠᶠ(i, j, k, grid, loc, κ::F, clock, args...) where F<:Function = κ(node(i, j, k, grid, f, f, c)..., clock.time)
...
@inline κᶠᶠᶜ(i, j, k, grid, loc, κ::DiscreteDiffusionFunction, clock, fields) = getdiffusivity(κ, i, j, k, grid, (f, f, c), clock, fields)
