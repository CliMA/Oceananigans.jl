...
function compute_diffusivities!(diffusivity_fields, closure::FlavorOfCATKE, model; parameters = :xyz)
    ...
    launch!(arch, grid, :xy,
            ...
            diffusivity_fields.Jᵇ, diffusivity_fields.hc,
            grid, closure, velocities, tracers, buoyancy, top_tracer_bcs, clock, Δt)
...
@kernel function compute_average_surface_buoyancy_flux!(Jᵇ, hc, grid, closure, velocities,
...
@inline function κuᶜᶜᶠ(i, j, k, grid, closure::FlavorOfCATKE, velocities, tracers, buoyancy, Jᵇ, hc)
    w★ = ℑzᵃᵃᶠ(i, j, k, grid, turbulent_velocityᶜᶜᶜ, closure, tracers.e)
    ℓu = momentum_mixing_lengthᶜᶜᶠ(i, j, k, grid, closure, velocities, tracers, buoyancy, Jᵇ, hc)
    κu = ℓu * w★
    κu_max = closure.maximum_viscosity
    return min(κu, κu_max)
end

@inline function κcᶜᶜᶠ(i, j, k, grid, closure::FlavorOfCATKE, velocities, tracers, buoyancy, Jᵇ, hc)
    w★ = ℑzᵃᵃᶠ(i, j, k, grid, turbulent_velocityᶜᶜᶜ, closure, tracers.e)
    ℓc = tracer_mixing_lengthᶜᶜᶠ(i, j, k, grid, closure, velocities, tracers, buoyancy, Jᵇ, hc)
    κc = ℓc * w★
    κc_max = closure.maximum_tracer_diffusivity
    return min(κc, κc_max)
end

@inline function κeᶜᶜᶠ(i, j, k, grid, closure::FlavorOfCATKE, velocities, tracers, buoyancy, Jᵇ, hc)
    w★ = ℑzᵃᵃᶠ(i, j, k, grid, turbulent_velocityᶜᶜᶜ, closure, tracers.e)
    ℓe = TKE_mixing_lengthᶜᶜᶠ(i, j, k, grid, closure, velocities, tracers, buoyancy, Jᵇ, hc)
    κe = ℓe * w★
