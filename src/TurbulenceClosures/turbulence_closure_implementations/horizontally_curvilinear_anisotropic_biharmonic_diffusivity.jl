...
"""
struct HorizontallyCurvilinearAnisotropicBiharmonicDiffusivity{NHC, NHN, NZ, KH, KZ} <: AbstractTurbulenceClosure{ExplicitTimeDiscretization}
    νhᶜᶜᶜ :: NHC
    νhᶠᶠᶜ :: NHN
    νz :: NZ
    κh :: KH
...
=================
  ...
  - `κz`: Vertical diffusivity. `Number`, `AbstractArray`, or `Function(x, y, z, t)`, or
          `NamedTuple` of diffusivities with entries for each tracer.
"""
function HorizontallyCurvilinearAnisotropicBiharmonicDiffusivity(FT=Float64; νhᶜᶜᶜ=0, νhᶠᶠᶜ=0, κh=0, νz=nothing, κz=nothing)
    νhᶜᶜᶜ = convert_diffusivity(FT, νhᶜᶜᶜ)
    νhᶠᶠᶜ = convert_diffusivity(FT, νhᶠᶠᶜ)
    νz = convert_diffusivity(FT, νz)
    κh = convert_diffusivity(FT, κh)
    κz = convert_diffusivity(FT, κz)
    return HorizontallyCurvilinearAnisotropicBiharmonicDiffusivity(νhᶜᶜᶜ, νhᶠᶠᶜ, νz, κh, κz)
end
...
function with_tracers(tracers, closure::HorizontallyCurvilinearAnisotropicBiharmonicDiffusivity)
    ...
    return HorizontallyCurvilinearAnisotropicBiharmonicDiffusivity(closure.νhᶜᶜᶜ, closure.νhᶠᶠᶜ, closure.νz, κh, κz)
end

calculate_diffusivities!(diffusivities, closure::HorizontallyCurvilinearAnisotropicBiharmonicDiffusivity, args...) = nothing

const NoVerticalViscHCABD = HCABD{<:Any, <:Any, <:Nothing}
const NoVerticalDiffHCABD = HCABD{<:Any, <:Any, <:Any, <:Any, <:Nothing}

@inline viscous_flux_ux(i, j, k, grid, closure::HCABD, clock, U, args...) = + ν_δ★ᶜᶜᶜ(i, j, k, grid, clock, closure.νhᶜᶜᶜ, U.u, U.v)   
@inline viscous_flux_uy(i, j, k, grid, closure::HCABD, clock, U, args...) = - ν_ζ★ᶠᶠᶜ(i, j, k, grid, clock, closure.νhᶠᶠᶜ, U.u, U.v)   
@inline viscous_flux_uz(i, j, k, grid, closure::HCABD, clock, U, args...) = + ν_uzzzᶠᶜᶠ(i, j, k, grid, clock, closure.νz, U.u)

@inline viscous_flux_vx(i, j, k, grid, closure::HCABD, clock, U, args...) = + ν_ζ★ᶠᶠᶜ(i, j, k, grid, clock, closure.νhᶠᶠᶜ, U.u, U.v)
@inline viscous_flux_vy(i, j, k, grid, closure::HCABD, clock, U, args...) = + ν_δ★ᶜᶜᶜ(i, j, k, grid, clock, closure.νhᶜᶜᶜ, U.u, U.v)
...
@inline diffusive_flux_z(i, j, k, grid, closure::NoVerticalDiffHCABD, c, ::Val{tracer_index}, clock, args...) where tracer_index = zero(eltype(grid))
