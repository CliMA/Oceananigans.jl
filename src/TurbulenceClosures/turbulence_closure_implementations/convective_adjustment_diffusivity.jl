...
function DiffusivityFields(arch, grid, tracer_names, bcs, closure::CAVD)
    stable_buoyancy_gradient = ZFaceField(arch, grid, nothing)
...
@kernel function compute_stability!(diffusivities, grid, clock, closure, tracers, buoyancy)
...
@inline function κᶜᶜᶠ(i, j, k, grid, clock, closure::CAVD, stable_buoyancy_gradient, ::Val{tracer_index}) where tracer_index
    @inbounds stableᶜᶜᶠ = diffusivities.stable_buoyancy_gradient[i, j, k]
