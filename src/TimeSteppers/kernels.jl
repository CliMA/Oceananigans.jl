...
function calculate_interior_source_terms!(G, arch, grid, coriolis, surface_waves, closure, U, C, pHY′, K, F, parameters, time)
    ...
    @launch device(arch) threads=(Tx, Ty) blocks=(Bx, By, Bz) calculate_Gu!(G.u, grid, coriolis, surface_waves, closure, U, C, K, F,
                                                                            pHY′, parameters, time)

    @launch device(arch) threads=(Tx, Ty) blocks=(Bx, By, Bz) calculate_Gv!(G.v, grid, coriolis, surface_waves, closure, U, C, K, F,
                                                                            pHY′, parameters, time)

    @launch device(arch) threads=(Tx, Ty) blocks=(Bx, By, Bz) calculate_Gw!(G.w, grid, coriolis, surface_waves, closure, U, C, K, F,
                                                                            parameters, time)
