"""
    RK3TimeStepper{FT, TG, P} <: AbstractTimeStepper

Holds parameters and tendency data for a low storage, third-order Runge-Kutta-Wray
time-stepping scheme described by Le and Moin (1991).
"""
struct RK3TimeStepper{FT, TG, P} <: AbstractTimeStepper
    γ¹ :: FT
    γ² :: FT
    γ³ :: FT
    ζ² :: FT
...
function time_step!(model::IncompressibleModel{<:RK3TimeStepper}, Δt)
    ...
    rk3_substep!(velocities, tracers, model.architecture, model.grid, Δt, γ¹, nothing, Gⁿ, nothing)

    calculate_pressure_correction!(pressures.pNHS, Δt, velocities, model)
    fractional_step_velocities!(velocities, tracers, model.architecture, model.grid, Δt, pressures.pNHS)

    tick!(model.clock, γ¹ * Δt)
    ...
    rk3_substep!(velocities, tracers, model.architecture, model.grid, Δt, γ², ζ², Gⁿ, G⁻)

    calculate_pressure_correction!(pressures.pNHS, Δt, velocities, model)
    fractional_step_velocities!(velocities, tracers, model.architecture, model.grid, Δt, pressures.pNHS)

    tick!(model.clock, (γ² + ζ²) * Δt)
...
function rk3_substep!(U, C, arch, grid, Δt, γⁿ, ζⁿ, Gⁿ, G⁻)
    ...
    for i in 1:length(C)
        @inbounds c = C[i]
        @inbounds Gcⁿ = Gⁿ[i+3]
        @inbounds Gc⁻ = isnothing(G⁻) ? nothing : G⁻[i+3]
