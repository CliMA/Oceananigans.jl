...
export CUDABackendMine

struct CUDABackendMine <: KA.GPU
    prefer_blocks::Bool
    always_inline::Bool
    max_registers::Int
end

CUDABackendMine(; prefer_blocks=false, always_inline=false, max_registers=255) = CUDABackendMine(prefer_blocks, always_inline, max_registers)

KA.allocate(::CUDABackendMine, ::Type{T}, dims::Tuple) where T = CuArray{T}(undef, dims)
KA.zeros(::CUDABackendMine, ::Type{T}, dims::Tuple) where T = CUDA.zeros(T, dims)
KA.ones(::CUDABackendMine, ::Type{T}, dims::Tuple) where T = CUDA.ones(T, dims)

KA.get_backend(::CuArray) = CUDABackendMine()
KA.get_backend(::CUSPARSE.AbstractCuSparseArray) = CUDABackendMine()
KA.synchronize(::CUDABackendMine) = synchronize()

Adapt.adapt_storage(::CUDABackendMine, a::Array) = Adapt.adapt(CuArray, a)
Adapt.adapt_storage(::CUDABackendMine, a::CuArray) = a
Adapt.adapt_storage(::KA.CPU, a::CuArray) = convert(Array, a)

## memory operations

function KA.copyto!(::CUDABackendMine, A, B)
    A isa Array && __pin!(A)
    B isa Array && __pin!(B)
...
function KA.mkcontext(kernel::KA.Kernel{CUDABackendMine}, _ndrange, iterspace)
    KA.CompilerMetadata{KA.ndrange(kernel), KA.DynamicCheck}(_ndrange, iterspace)
end

function KA.launch_config(kernel::KA.Kernel{CUDABackendMine}, ndrange, workgroupsize)
...
function (obj::KA.Kernel{CUDABackendMine})(args...; ndrange=nothing, workgroupsize=nothing)
    ...
    kernel = @cuda launch=false always_inline=backend.always_inline maxthreads=maxthreads max_registers=backend.max_registers obj.f(ctx, args...)
...
KA.argconvert(k::KA.Kernel{CUDABackendMine}, arg) = cudaconvert(arg)

function KA.priority!(::CUDABackendMine, prio::Symbol)
