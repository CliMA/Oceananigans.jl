...
"""
    RegularCartesianGrid([T=Float64]; N, x, y, z)

Creates a `RegularCartesianGrid` with `N = (Nx, Ny, Nz)` grid points and domain given by
`x = (x₁, x₂)`, `y = (y₁, y₂)`, and `z = (z₁, z₂)` where constants are stored using
...
function RegularCartesianGrid(T; N, x, y, z)
    ...
    # usually use second-order advection schemes so halos of size Hx, Hy, Hz = 1
