...
function write_output(model::Model{arch}, chk::Checkpointer) where arch <: Architecture
    ...
    model.poisson_solver = init_poisson_solver(arch(), model.grid, model.timestepper.tmp.fCC1)
...
function restore_from_checkpoint(filepath)
    ...
    model.poisson_solver = init_poisson_solver(arch(model)(), model.grid, model.timestepper.tmp.fCC1)
