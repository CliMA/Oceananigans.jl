...
function inject_halo_communication_boundary_conditions(field_bcs, local_rank, connectivity, topology)
    rank_east   = connectivity.eas
    rank_west   = connectivity.wes
    ...
    # `LeftConnected` means bounded on the righ
    ...
    west  = inject_west  ? west_comm_bc  : field_bcs.wes
    east  = inject_east  ? east_comm_bc  : field_bcs.eas
