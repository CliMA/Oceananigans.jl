...
using Oceananigans.Grids: topology, validate_tupled_argumen
...
import Oceananigans.Utils: sync_device!, tuplei
...
- `partition`: A [`Partition`](@ref) specifying the total processors in the `x`, `y`, and `z` direction.
               Note that support for distributed `z` direction is  limited; we strongly sugges
