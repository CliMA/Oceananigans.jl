...
import Oceananigans.ImmersedBoundaries: compute_active_cells
...
function compute_active_cells(grid::DistributedGrid, ib)
    ...
    if arch isa SynchronizedDistributed
        return compute_interior_active_cells(grid, ib; parameters = :xyz)
    end
    ...
    west_halo_dependent_cells  = compute_interior_active_cells(grid, ib; parameters = KernelParameters(west_boundary...))
    east_halo_dependent_cells  = compute_interior_active_cells(grid, ib; parameters = KernelParameters(east_boundary...))
    south_halo_dependent_cells = compute_interior_active_cells(grid, ib; parameters = KernelParameters(south_boundary...))
    north_halo_dependent_cells = compute_interior_active_cells(grid, ib; parameters = KernelParameters(north_boundary...))
    ...
    halo_independent_cells = compute_interior_active_cells(grid, ib; parameters = KernelParameters((nx, ny, Nz), (ox, oy, 0)))
