...
In our implementation we require `Nz ≥ Ry` and `Nx ≥ Ry` with the additional constrain
...
The 'slab' decomposition works in the same manner while skipping the transposes tha
