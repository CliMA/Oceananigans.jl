...
function tupled_fill_halo_regions!(fields, args...; kwargs...)
    non_reduced_fields = fill_windowed_and_reduced_fields!(fields, args...; kwargs)
  
    if !isempty(non_reduced_fields) # ie not reduced, and with default_indices
        grid = first(non_reduced_fields).grid
        fill_halo_regions!(map(data, non_reduced_fields),
                           map(boundary_conditions, non_reduced_fields),
                           default_indices(3),
                           map(instantiated_location, non_reduced_fields),
                           grid, args...; kwargs...)
    end
...
"""
    fill_windowed_and_reduced_fields!(fields, args...; kwargs)

Fill the halos of WindowedField and ReducedFields, and return the rest of the fields
in a tuple.
"""
@inline function fill_windowed_and_reduced_fields!(fields, args...; kwargs)
    non_reduced_fields = Field[]
    ...
    for f in fields
        if !isnothing(boundary_conditions(f))
            if f isa ReducedField || !(f isa FullField)
                ...
                push!(non_reduced_fields, f)
            end
        end
    end

    return tuple(non_reduced_fields...)
end
