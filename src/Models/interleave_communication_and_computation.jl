...
"""
    all_halo_surface_kernel_parameters(grid)

Return kernel parameters for computing 2D (surface) variables over the full halo region.

Unlike `surface_kernel_parameters`, which excludes the outermost halo cell,
this covers all halo cells (indices `-Hx+1:Nx+Hx` and `-Hy+1:Ny+Hy`).
"""
@inline function all_halo_surface_kernel_parameters(grid)
    Nx, Ny, _ = size(grid)
    Hx, Hy, _ = halo_size(grid)
    Tx, Ty, _ = topology(grid)
...
"""
    all_halo_volume_kernel_parameters(grid)

Return kernel parameters for computing 3D (volume) variables over the full halo region.

Unlike `volume_kernel_parameters`, which excludes the outermost halo cell,
this covers all halo cells (indices `-Hx+1:Nx+Hx`, `-Hy+1:Ny+Hy`, `-Hz+1:Nz+Hz`).
"""
@inline function all_halo_volume_kernel_parameters(grid)
    Nx, Ny, Nz = size(grid)
    Hx, Hy, Hz = halo_size(grid)
    Tx, Ty, Tz = topology(grid)
    ...
    ii = ifelse(Tx == Flat, 1:Nx, -Hx+1:Nx+Hx)
    jj = ifelse(Ty == Flat, 1:Ny, -Hy+1:Ny+Hy)
    kk = ifelse(Tz == Flat, 1:Nz, -Hz+1:Nz+Hz)
