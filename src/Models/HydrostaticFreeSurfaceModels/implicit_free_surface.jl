...
function FreeSurface(free_surface::ImplicitFreeSurface{Nothing}, velocities, grid)
    ...
    solver = build_implicit_step_solver(Val(solver_method), grid, free_surface.solver_settings)
...
function build_implicit_step_solver(::Val{:Default}, grid, settings)
    default_method = is_horizontally_regular(grid) ? :FastFourierTransform : :PreconditionedConjugateGradient
    return build_implicit_step_solver(Val(default_method), grid, settings)
end
