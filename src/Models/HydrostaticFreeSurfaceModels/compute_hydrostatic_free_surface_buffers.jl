...
function compute_buffer_tendency_contributions!(grid::DistributedActiveInteriorIBG, arch, model)
    # Loop over boundary regions: west, east, south, north
    for region in (:west, :east, :south, :north)
        # The region keyword will resolve to the appropriate halo-dependent cells map.
        # If the map is nothing (no processor boundary), the kernel becomes a no-op.
        compute_hydrostatic_free_surface_tendency_contributions!(model, :xyz; region)
    end
