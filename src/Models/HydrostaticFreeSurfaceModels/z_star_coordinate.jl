...
@inline pressure_gradient_x(i, j, k, grid, p, b, C) = ∂xᶠᶜᶜ(i, j, k, grid, p)
@inline pressure_gradient_y(i, j, k, grid, p, b, C) = ∂yᶜᶠᶜ(i, j, k, grid, p)

@inline pressure_gradient_x(i, j, k, grid::MutableGridOfSomeKind, p, b, C) = ∂xᶠᶜᶜ(i, j, k, grid, p) + grid_slope_contribution_x(i, j, k, grid, b, C)
@inline pressure_gradient_y(i, j, k, grid::MutableGridOfSomeKind, p, b, C) = ∂yᶜᶠᶜ(i, j, k, grid, p) + grid_slope_contribution_y(i, j, k, grid, b, C)
...
@inline grid_slope_contribution_x(i, j, k, grid::MutableGridOfSomeKind, ::Nothing, model_fields) = zero(grid)
@inline grid_slope_contribution_y(i, j, k, grid::MutableGridOfSomeKind, ::Nothing, model_fields) = zero(grid)

@inline grid_slope_contribution_x(i, j, k, grid::MutableGridOfSomeKind, buoyancy, model_fields) = ℑxᶠᵃᵃ(i, j, k, grid, buoyancy_perturbationᶜᶜᶜ, buoyancy.formulation, model_fields) * ∂x_z(i, j, k, grid)
@inline grid_slope_contribution_y(i, j, k, grid::MutableGridOfSomeKind, buoyancy, model_fields) = ℑyᵃᶠᵃ(i, j, k, grid, buoyancy_perturbationᶜᶜᶜ, buoyancy.formulation, model_fields) * ∂y_z(i, j, k, grid)
