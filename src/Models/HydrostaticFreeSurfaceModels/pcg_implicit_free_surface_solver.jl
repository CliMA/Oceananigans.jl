...
using Oceananigans.Grids: isrectilinear, halo_size, SphericalShellGrid
...
compute_regional_rhs!(rhs, arch, grid::SphericalShellGrid, g, Δt, U, η) =
    launch!(arch, grid, :xy,
            implicit_free_surface_right_hand_side_nonorthogonal!,
            rhs, grid, g, Δt, U, η)
...
@inline function implicit_free_surface_barotropic_flux_u(i, j, k, grid, u, v)
    Ū = barotropic_U(i, j, k, grid, u)
    V̄ = barotropic_V(i, j, k, grid, v)
...
@inline function implicit_free_surface_barotropic_flux_v(i, j, k, grid, u, v)
    Ū = barotropic_U(i, j, k, grid, u)
    V̄ = barotropic_V(i, j, k, grid, v)
...
@kernel function implicit_free_surface_right_hand_side_nonorthogonal!(rhs, grid, g, Δt, U, η)
    i, j = @index(Global, NTuple)
    kᴺ   = grid.Nz
    Az   = Azᶜᶜᶠ(i, j, kᴺ, grid)
    u = Oceananigans.Advection.u_velocity(U)
    v = Oceananigans.Advection.v_velocity(U)
    δx_U = δxᶜᵃᵃ(i, j, kᴺ, grid, implicit_free_surface_barotropic_flux_u, u, v)
    δy_V = δyᵃᶜᵃ(i, j, kᴺ, grid, implicit_free_surface_barotropic_flux_v, u, v)
    @inbounds rhs[i, j, kᴺ+1] = (δx_U + δy_V - Az * η[i, j, kᴺ+1] / Δt) / (g * Δt)
end
...
@inline ∫ᶻ_Ax_∇ηᶠᶜᶜ(i, j, k, grid::SphericalShellGrid, ∫ᶻ_Axᶠᶜᶜ, η) =
    @inbounds ∫ᶻ_Axᶠᶜᶜ[i, j, k] *
              Oceananigans.Operators.covariant_gradient_xᶠᶜᶜ(i, j, k, grid, η)

@inline ∫ᶻ_Ay_∇ηᶜᶠᶜ(i, j, k, grid::SphericalShellGrid, ∫ᶻ_Ayᶜᶠᶜ, η) =
    @inbounds ∫ᶻ_Ayᶜᶠᶜ[i, j, k] *
              Oceananigans.Operators.covariant_gradient_yᶜᶠᶜ(i, j, k, grid, η)

@inline Az_∇h²ᶜᶜᶜ(i, j, k, grid::SphericalShellGrid, ∫ᶻ_Axᶠᶜᶜ, ∫ᶻ_Ayᶜᶠᶜ, η) =
    (δxᶜᵃᵃ(i, j, k, grid, ∫ᶻ_Ax_∇ηᶠᶜᶜ, ∫ᶻ_Axᶠᶜᶜ, η) +
     δyᵃᶜᵃ(i, j, k, grid, ∫ᶻ_Ay_∇ηᶜᶠᶜ, ∫ᶻ_Ayᶜᶠᶜ, η))
...
struct ImplicitFreeSurfaceBasisField{FT, I}
    i₀ :: I
    j₀ :: I
    k₀ :: I
end

@inline Base.getindex(η::ImplicitFreeSurfaceBasisField{FT}, i, j, k) where FT =
    ifelse(i == η.i₀ && j == η.j₀ && k == η.k₀, one(FT), zero(FT))
...
@inline function local_implicit_free_surface_diagonal_coefficient(i, j, k, grid::SphericalShellGrid, ax, ay, g, Δt)
    η = ImplicitFreeSurfaceBasisField{typeof(zero(grid)), typeof(i)}(i, j, k)
    Az = Azᶜᶜᶜ(i, j, 1, grid)
    return Az_∇h²ᶜᶜᶜ(i, j, k, grid, ax, ay, η) - Az / (g * Δt^2)
end

@inline function heuristic_inverse_times_residuals(i, j, r, grid::SphericalShellGrid, g, Δt, ax, ay)
    diagonal = local_implicit_free_surface_diagonal_coefficient(i, j, grid.Nz+1, grid, ax, ay, g, Δt)
    @inbounds return ifelse(diagonal == zero(grid), zero(grid), r[i, j, grid.Nz+1] / diagonal)
end
