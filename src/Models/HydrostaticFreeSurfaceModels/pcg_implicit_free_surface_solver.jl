...
function implicit_free_surface_linear_operation!(L_ηⁿ⁺¹, ηⁿ⁺¹, ∫ᶻ_Axᶠᶜᶜ, ∫ᶻ_Ayᶜᶠᶜ, g, Δt)
    ...
    # Note: because of `fill_halo_regions!` below, we cannot use `PCGImplicitFreeSurface` on a multi-region grid
    # (`fill_halo_regions!` cannot be used within `@apply_regionally`)
