...
function calculate_hydrostatic_momentum_tendencies!(model, velocities; dependencies = device_event(model))
    ...
    Gu_event = launch!(arch, grid, :xyz,
                       calculate_hydrostatic_free_surface_Gu!, model.timestepper.Gⁿ.u, only_active_cells, u_kernel_args...;
                       dependencies = dependencies, only_active_cells)

    Gv_event = launch!(arch, grid, :xyz,
                       calculate_hydrostatic_free_surface_Gv!, model.timestepper.Gⁿ.v, only_active_cells, v_kernel_args...;
                       dependencies = dependencies, only_active_cells)
...
@kernel function calculate_hydrostatic_free_surface_Gu!(Gu, only_active_cells, grid, args...)
    idx        = @index(Global, Linear)
    i′, j′, k′ = @index(Global, NTuple)
    i, j, k = calc_tendency_index(idx, i′, j′, k′, grid, Val(only_active_cells))
    @inbounds Gu[i, j, k] = hydrostatic_free_surface_u_velocity_tendency(i, j, k, grid, args...)
end

""" Calculate the right-hand-side of the v-velocity equation. """
@kernel function calculate_hydrostatic_free_surface_Gv!(Gv, only_active_cells, grid, args...)
    idx        = @index(Global, Linear)
    i′, j′, k′ = @index(Global, NTuple)
    i, j, k = calc_tendency_index(idx, i′, j′, k′, grid, Val(only_active_cells))
...
@kernel function calculate_hydrostatic_free_surface_Gc!(Gc, tendency_kernel_function, only_active_cells, grid, args...)
    idx        = @index(Global, Linear)
    i′, j′, k′ = @index(Global, NTuple)
    i, j, k = calc_tendency_index(idx, i′, j′, k′, grid, Val(only_active_cells))
