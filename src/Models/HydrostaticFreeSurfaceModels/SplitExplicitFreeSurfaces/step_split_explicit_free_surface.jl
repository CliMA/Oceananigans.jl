...
function iterate_split_explicit!(free_surface, grid, GUⁿ, GVⁿ, Δτᴮ, weights, ::Val{Nsubsteps}) where Nsubsteps
    ...
    η_kernel!, _ = configure_kernel(arch, grid, parameters, _split_explicit_free_surface!)
    U_kernel!, _ = configure_kernel(arch, grid, parameters, _split_explicit_barotropic_velocity!)
