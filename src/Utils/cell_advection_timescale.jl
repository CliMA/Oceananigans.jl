...
@inline y_cell_advection_timescale(grid::RegularCartesianGrid, vmax) = grid.Δy / vmax
@inline z_cell_advection_timescale(grid::RegularCartesianGrid, wmax) = grid.Δz / wmax

x_cell_advection_timescale(grid::RegularCartesianGrid{FT, <:Flat, TY, TZ}, umax) where {FT, TY, TZ} = Inf
y_cell_advection_timescale(grid::RegularCartesianGrid{FT, TX, <:Flat, TZ}, vmax) where {FT, TX, TZ} = Inf
z_cell_advection_timescale(grid::RegularCartesianGrid{FT, TX, TY, <:Flat}, wmax) where {FT, TX, TY} = Inf
