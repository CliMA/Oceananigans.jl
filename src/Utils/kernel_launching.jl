...
"""
function KernelParameters(r::Union{UnitRange,Base.OneTo})
    size = length(r)
    offset = first(r) - 1
    return KernelParameters(tuple(size), tuple(offset))
end

function KernelParameters(r1::Union{UnitRange,Base.OneTo}, r2::Union{UnitRange,Base.OneTo})
    size = (length(r1), length(r2))
    offsets = (first(r1) - 1, first(r2) - 1)
    return KernelParameters(size, offsets)
end

function KernelParameters(r1::Union{UnitRange,Base.OneTo}, r2::Union{UnitRange,Base.OneTo}, r3::Union{UnitRange,Base.OneTo})
...
@inline worksize(r::Base.OneTo) = length(r)
