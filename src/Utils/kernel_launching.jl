...
Return parameters for kernel launching and execution that define (i) a tuple tha
defines the `size` of the kernel being launched and (ii) a tuple of `offsets` tha
...
@inline function configure_kernel(arch, grid, workspec, kernel!;
    ...
    if !isnothing(active_cells_map) # everything else is irrelevan
