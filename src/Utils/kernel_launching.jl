...
"""
    work_layout(grid, worksize::NTuple{N, Int}; kwargs...)

Infer `workgroup` from `grid` and return `workgroup, worksize`.
"""
...
"""
    work_layout(grid, workdims::Symbol; include_right_boundaries=false, location=nothing)
...
* `location`: Tailor the kernel size to `location`. Only matters if `include_right_boundaries`
              is also specified. (Default: `nothing`.)

* `include_right_boundaries`: Include the "right" boundary at `N+1` in the kernel index range.
                              Requires `location` and only matters if at least one of `location`
                              is `Face` (Default: `false`).

* `reduced_dimensions`: Indicate dimensions to be removed from the `worksize`.
...
function launch!(arch, grid, workspec, kernel!, kernel_args...;
    ...
    event = loop!(kernel_args...; dependencies=dependencies, kwargs...)
