...
@inline two_Ωʸw_minus_two_Ωᶻv(i, j, k, grid, coriolis, U) =
     (  two_Ωʸ(coriolis, ynode(j, grid, Center()), znode(k, grid, Center())) * ℑzᵃᵃᶜ(i, j, k, grid, U.w)
      - two_Ωᶻ(coriolis, ynode(j, grid, Center()), znode(k, grid, Center())) * ℑyᵃᶜᵃ(i, j, k, grid, U.v))
...
@inline y_f_cross_U(i, j, k, grid, coriolis::NonTraditionalBetaPlane, U) =
    two_Ωᶻ(coriolis,ynode(j, grid, Face()), znode(k, grid, Center())) * ℑxyᶜᶠᵃ(i, j, k, grid, U.u)

@inline z_f_cross_U(i, j, k, grid, coriolis::NonTraditionalBetaPlane, U) =
   -two_Ωʸ(coriolis, ynode(j, grid, Center()), znode(k, grid, Face())) * ℑxzᶜᵃᶠ(i, j, k, grid, U.u)
