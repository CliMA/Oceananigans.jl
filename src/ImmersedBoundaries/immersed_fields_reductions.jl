...
@inline condition_operand(f::Function, op::ImmersedField, condition, mask) = ConditionalOperation(op; f, condition = NotImmersed(condition), mask)
@inline condition_operand(f::Function, op::ImmersedField, ::Nothing, mask) = ConditionalOperation(op; f, condition = NotImmersed(truefunc), mask)
