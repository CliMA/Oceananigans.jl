...
function x_derivative_cell(FT, arch)
    grid = RegularRectilinearOrthogonalGrid(FT, size=(3, 3, 3), extent=(3, 3, 3))
...
function computation_including_boundaries(FT, arch)
    ...
    grid = RegularRectilinearOrthogonalGrid(FT, topology=topo, size=(13, 17, 19), extent=(1, 1, 1))
...
function computations_with_buoyancy_field(FT, arch, buoyancy)
    grid = RegularRectilinearOrthogonalGrid(FT, size=(1, 1, 1), extent=(1, 1, 1))
...
@testset "Abstract operations" begin
    ...
    @testset "Combined binary operations and derivatives" begin
        ...
        for FT in float_types
            grid = RegularRectilinearOrthogonalGrid(FT, size=(Nx, Nx, Nx), extent=(Nx, Nx, Nx))
    ...
    for arch in archs
        @testset "AbstractOperations and ComputedFields [$(typeof(arch))]" begin
            ...
            for FT in (Float64,) #float_types
                ...
                grid = RegularRectilinearOrthogonalGrid(FT, size=(4, 4, 4), extent=(1, 1, 1),
