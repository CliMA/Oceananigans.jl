...
function run_thermal_bubble_netcdf_tests()
    ...
    @test all(u .≈ model.velocities.u.data[1:Nx, 1:Ny, 1:Nz])
    @test all(v .≈ model.velocities.v.data[1:Nx, 1:Ny, 1:Nz])
    @test all(w .≈ model.velocities.w.data[1:Nx, 1:Ny, 1:Nz])
    @test all(T .≈ model.tracers.T.data[1:Nx, 1:Ny, 1:Nz])
    @test all(S .≈ model.tracers.S.data[1:Nx, 1:Ny, 1:Nz])
end
