...
@testset "Reactant LatitudeLongitudeGrid Simulation Tests" begin
    ...
    hydrostatic_model_kw = (momentum_advection = WENOVectorInvariant(order=5),
                            free_surface=SplitExplicitFreeSurface(substeps=4),
                            tracer_advection = WENO(order=5),
                            tracers = (:T, :S, :e),
                            buoyancy = SeawaterBuoyancy(; equation_of_state),
                            closure = CATKEVerticalDiffusivity())

    @info "Testing LatitudeLongitudeGrid + WENO + CATKE + SplitExplicitFreeSurface \
             + HydrostaticFreeSurfaceModel Reactant correctness"
    simulation = test_reactant_model_correctness(LatitudeLongitudeGrid,
                                                 HydrostaticFreeSurfaceModel,
                                                 lat_lon_kw,
                                                 hydrostatic_model_kw,
