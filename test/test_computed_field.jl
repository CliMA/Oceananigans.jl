...
function compute_unary(unary, model)
    set!(model; S=π)
    T, S = model.tracers
    @compute uS = Field(unary(S), data=model.pressure.data)
...
function compute_plus(model)
    set!(model; S=π, T=42)
    T, S = model.tracers
    @compute ST = Field(S + T, data=model.pressure.data)
...
function compute_kinetic_energy(model)
    ...
    @compute kinetic_energy = Field(kinetic_energy_operation, data=model.pressure.data)
