...
#####
##### Build a model for plankton
#####

using Oceananigans.Grids: znode
import Oceananigans.Biogeochemistry: 
    validate_biogeochemistry,
    get_biogeochemical_forcing

struct SimplePlanktonGrowthDeath
    μ₀ :: Float64
    λ :: Float64
    m :: Float64
end

validate_biogeochemistry(::SimplePlanktonGrowthDeath, tracernames) = :P ∈ tracernames

@inline function growing_and_grazing(i, j, k, grid, clock, bgc, fields)
    z = znode(Center(), k, grid)
    P = @inbounds fields.P[i, j, k]
    return (bgc.μ₀ * exp(z / bgc.λ) - bgc.m) * P
end

get_biogeochemical_forcing(::SimplePlanktonGrowthDeath, ::Val{:P}) = 
    growing_and_grazing
...
model = NonhydrostaticModel(; grid,
                            ...
                            biogeochemistry = SimplePlanktonGrowthDeath(1/day, 5.0, 0.1/day),
