...
const ZARR_MPI_WORKER = joinpath(@__DIR__, "..", "setup", "distributed_zarr_writer_tests.jl")
...
@testset "ZarrWriter [MPI]" begin
    ...
    @testset "Partition(x=2)" begin
        path = abspath("test_dist_zarr_x.zarr")
    ...
    @testset "Partition(y=2)" begin
        path = abspath("test_dist_zarr_y.zarr")
    ...
    @testset "Partition(x=2, y=2)" begin
        path = abspath("test_dist_zarr_xy.zarr")
