...
function test_diffusion_simple(fieldname, timestepper)
    ...
    model = IncompressibleModel(timestepper = timestepper,
                                       grid = RegularRectilinearOrthogonalGrid(size=(1, 1, 16), extent=(1, 1, 1)),
...
function test_diffusion_cosine(fieldname, timestepper)
    ...
    grid = RegularRectilinearOrthogonalGrid(size=(1, 1, Nz), x=(0, 1), y=(0, 1), z=(0, Lz))
...
function internal_wave_test(timestepper; N=128, Nt=10, background_stratification=false)
    ...
    model = IncompressibleModel(timestepper = timestepper,
                                       grid = RegularRectilinearOrthogonalGrid(size=(N, 1, N), extent=(L, L, L)),
...
function passive_tracer_advection_test(timestepper; N=128, κ=1e-12, Nt=100, background_velocity_field=false)
    ...
    grid = RegularRectilinearOrthogonalGrid(size=(N, N, 2), extent=(L, L, L))
    closure = IsotropicDiffusivity(ν=κ, κ=κ)
    ...
    @testset "Budgets in isotropic diffusion" begin
        ...
        for timestepper in timesteppers
            for topology in ((Periodic, Periodic, Periodic),
                ...
                grid = RegularRectilinearOrthogonalGrid(size=(4, 4, 4), extent=(1, 1, 1), topology=topology)
    ...
    @testset "Budgets in biharmonic diffusion" begin
        ...
        for timestepper in timesteppers
            for topology in ((Periodic, Periodic, Periodic),
                ...
                grid = RegularRectilinearOrthogonalGrid(size=(2, 2, 2), extent=(1, 1, 1), topology=topology)
