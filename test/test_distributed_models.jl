...
function test_complex_boundary_conditions(Rx, Ry, child_arch)
    ...
    model = HydrostaticFreeSurfaceModel(; grid, boundary_conditions=(; u=u_bcs), timestepper=:QuasiAdamsBashforth2, free_surface=SplitExplicitFreeSurface(grid; substeps=1, defaults.gravitational_acceleration))
