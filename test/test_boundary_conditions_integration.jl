...
function test_boundary_condition(arch, FT, topo, side, field_name, boundary_condition)
    grid = RegularRectilinearOrthogonalGrid(FT, size=(1, 1, 1), extent=(1, π, 42), topology=topo)
...
function test_flux_budget(arch, FT, fldname)
    N, κ, Lz = 16, 1, 0.7
    grid = RegularRectilinearOrthogonalGrid(FT, size=(N, N, N), extent=(1, 1, Lz))
...
function fluxes_with_diffusivity_boundary_conditions_are_correct(arch, FT)
    ...
    grid = RegularRectilinearOrthogonalGrid(FT, size=(16, 16, 16), extent=(1, 1, Lz))
...
@testset "Boundary condition integration tests" begin
    ...
    @testset "Boundary condition regularization" begin
        ...
        # yet.
        grid = RegularRectilinearOrthogonalGrid(FT, size=(1, 1, 1), extent=(1, π, 42), topology=(Periodic, Bounded, Bounded))
