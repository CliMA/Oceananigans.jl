...
@testset "Models" begin
    ...
    @testset "Adjustment of halos in NonhydrostaticModel constructor" begin
        ...
        for scheme in (Centered(order=4), UpwindBiased(order=3))
            model = NonhydrostaticModel(minimal_grid; momentum_advection=scheme, tracer_advection=scheme)
            @test model.grid.Hx == 2 && model.grid.Hy == 2 && model.grid.Hz == 2

            model = NonhydrostaticModel(funny_grid; momentum_advection=scheme, tracer_advection=scheme)
