...
@testset "reactnt correctness" begin
    ...
    @testset "compute_simple_Gu! correctness" begin
        ...
        @testset "RectilinearGrid" begin
            kw = (size=(4, 4, 4), halo=(3, 3, 3), extent=(1, 1, 1), topology=(Bounded, Bounded, Bounded))
            vanilla_grid = RectilinearGrid(vanilla_arch; kw...)
            reactnt_grid = RectilinearGrid(reactnt_arch; kw...)
        ...
        @testset "LatitudeLongitudeGrid" begin
            ...
            # Note: WENO with raise=true fails on grids with Bounded x-direction.
            # This is because u-momentum advection requires x-direction symmetric interpolations
            # for ALL THREE flux terms (Uu, Vu, Wu) due to staggering. When x is Bounded, this
            # creates deeply nested buffer scheme conditionals that overwhelm MLIR's pass manager.
            # See: src/Advection/upwind_biased_advective_fluxes.jl
            # Workaround: Use WENO(buffer_scheme=Centered(order=2)) to reduce nesting depth,
            # or use Centered() for advection when autodiff is required.
            @testset "coriolis=nothing" begin
                ...
                for advection in advection_schemes, raise in raise_modes
                    @testset "advection=$(adv_name(advection)) raise=$raise" begin
                        ...
                        fill!(reactnt_Gu, 0)
                        compute_simple_Gu!(vanilla_Gu, advection, nothing, vanilla_velocities)
                        if advection isa WENO && raise
                            @test_broken false  # MLIR pass manager fails with Bounded x-direction
                        else
                            @jit raise=raise compute_simple_Gu!(reactnt_Gu, advection, nothing, reactnt_velocities)
                            @test compare_interior("Gu", vanilla_Gu, reactnt_Gu)
                        end
