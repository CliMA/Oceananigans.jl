...
function test_function_differentiation(T=Float64)
    ...
    stretched_f = [0, 1, 3, 6]
    stretched_c = [-0.5, 0.5, 2, 4.5, 7.5] 
    
    dc(i) = stretched_f[i+1] - stretched_f[i]
    df(i) = stretched_c[i]   - stretched_c[i-1]

    grid  = RectilinearGrid(CPU(), T; size=(3, 3, 3), x=stretched_f, y=stretched_f, z=stretched_f)

    ∂x_f(i, j, k) = (ϕ²[i+1, j, k] - ϕ²[i, j, k]) / df(i)
    ∂x_c(i, j, k) = (ϕ²[i, j, k] - ϕ²[i-1, j, k]) / dc(i)
    ∂y_f(i, j, k) = (ϕ²[i, j+1, k] - ϕ²[i, j, k]) / df(j)
    ∂y_c(i, j, k) = (ϕ²[i, j, k] - ϕ²[i, j-1, k]) / dc(j)
    ∂z_f(i, j, k) = (ϕ²[i, j, k+1] - ϕ²[i, j, k]) / df(k)
    ∂z_c(i, j, k) = (ϕ²[i, j, k] - ϕ²[i, j, k-1]) / dc(k)

    for ∂x in (∂xᶜᶜᶜ, ∂xᶜᶜᶠ, ∂xᶜᶠᶜ, ∂xᶜᶠᶠ)
        @show ∂x
        @show assess = assess && ∂x(2, 2, 2, grid, f, ϕ) == ∂x_c(2, 2, 2) 
    end
    for ∂x in (∂xᶠᶜᶜ, ∂xᶠᶜᶠ, ∂xᶠᶠᶜ, ∂xᶠᶠᶠ)
