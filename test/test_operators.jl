function test_function_differentiation(T=Float64)
    grid = RegularRectilinearOrthogonalGrid(T; size=(3, 3, 3), extent=(3, 3, 3))
    ϕ = rand(T, 3, 3, 3)
    ϕ² = ϕ.^2
...
@testset "Operators" begin
    ...
    @testset "Grid lengths, areas, and volume operators" begin
        ...
        FT = Float64
        grid = RegularRectilinearOrthogonalGrid(FT, size=(1, 1, 1), extent=(π, 2π, 3π))
    ...
    @testset "2D operators" begin
        ...
        grid_yz = RegularRectilinearOrthogonalGrid(size=(1, Ny, Nz), extent=(Lx, Ly, Lz))
        ...
        grid_xz = RegularRectilinearOrthogonalGrid(size=(Nx, 1, Nz), extent=(Lx, Ly, Lz))
