...
function run_first_AB2_time_step_tests(arch, ft)
    ...
    @test all(model.timestepper.G.Gu.data .≈ 0)
    @test all(model.timestepper.G.Gv.data .≈ 0)
    @test all(model.timestepper.G.Gw.data .≈ 0)
    @test all(model.timestepper.G.GT.data .≈ 1.0)
    @test all(model.timestepper.G.GS.data .≈ 0)
