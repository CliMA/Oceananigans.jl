...
simulation.output_writers[:height] =
    NetCDFOutputWriter(model, model.solution, filepath = "one_dimensional_shallow_water.nc",
...
println("Saving plot of numerical and exact solutions.")

#herror = (interior(model.solution.h)[:, 1, 1] - hexact)/amp
#plt = plot(xn, herror, linewidth=2,
#            label="relative error", title=@sprintf("t = %.3f", model.clock.time))
