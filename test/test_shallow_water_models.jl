...
function time_stepping_shallow_water_model_works(arch, topo, coriolis, advection; timestepper=:RungeKutta3)
    grid = RectilinearGrid(size=(1, 1), extent=(2π, 2π), topology=topo)
...
function shallow_water_model_tracers_and_forcings_work(arch)
    grid = RectilinearGrid(size=(1, 1), extent=(2π, 2π), topology=((Periodic, Periodic, Flat)))
...
@testset "Shallow Water Models" begin
    ...
    @info "  Testing $topo model construction..."
        for arch in archs, FT in float_types                
            grid = RectilinearGrid(FT, topology=topo, size=(), extent=())
    ...
    for topo in topos
        @testset "$topo model construction" begin
            ...
            for arch in archs, FT in float_types
                ...
                grid = RectilinearGrid(FT, topology=topo, size=1, extent=1, halo=3)
                model = ShallowWaterModel(grid=grid, gravitational_acceleration=1, architecture=arch) 
    ...
    for topo in topos
        @testset "$topo model construction" begin
            ...
            for arch in archs, FT in float_types
                ...
                grid = RectilinearGrid(FT, topology=topo, size=(1, 1), extent=(1, 2), halo=(3, 3))
                model = ShallowWaterModel(grid=grid, gravitational_acceleration=1, architecture=arch)
    ...
    @info "  Testing setting shallow water model fields..."
        for arch in archs, FT in float_types
            ...
            grid = RectilinearGrid(FT, size=N, extent=L, topology=(Periodic, Periodic, Flat), halo=(3, 3))
            model = ShallowWaterModel(grid=grid, gravitational_acceleration=1, architecture=arch)
