...
drag_bc(implicit, λ, c★) = implicit ?
    ImplicitExplicitFluxBoundaryCondition(drag_explicit_part; coefficient=drag_coefficient, discrete_form=true, parameters=(; λ, c★)) :
...
function catke_drag_column(arch, Δt, nsteps; implicit, λ=0.05, u₀=1.0)
    ...
    if implicit   # drag toward 0: explicit part 0, coefficient λ (β = λ Δt / Δz = 5 at Δt = 100)
        u_top = ImplicitExplicitFluxBoundaryCondition(0.0; coefficient=λ)
        v_top = ImplicitExplicitFluxBoundaryCondition(0.0; coefficient=λ)
    else
