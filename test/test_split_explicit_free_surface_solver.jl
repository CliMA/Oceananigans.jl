...
@kernel function supertest2!(η)
    i, j = @index(Global, NTuple)
    η[i, j, 1] = 0.0
end

@kernel function supertest3!(sefs)
    i, j = @index(Global, NTuple)
    sefs.state.η[i, j, 1] = 0.0
end

@kernel function supertest4!(sefs)
    i, j = @index(Global, NTuple)
    sefs.η[i, j, 1] = 1.0
end
...
launch!(arch, grid, :xy, supertest2!, sefs.state.η)
launch!(arch, grid, :xy, supertest3!, sefs)

launch!(arch, grid, :xy, supertest4!, sefs)
...
const split_explicit_state_fieldnames = fieldnames(SplitExplicitState)
function Base.getproperty(free_surface::SplitExplicitFreeSurface{S}, sym::Symbol) where {S<:SplitExplicitState}
...
=#
const split_explicit_state_fieldnames = fieldnames(SplitExplicitState)
const split_explicit_auxiliary_fieldnames = fieldnames(SplitExplicitAuxiliary)
const split_explicit_setting_fieldnames = fieldnames(SplitExplicitSettings)
