...
advection(formulation::VectorInvariantFormulation) = VectorInvariant(vorticity_scheme = WENO(), divergence_scheme = nothing, vertical_scheme = EnergyConserving())
...
function run_shallow_water_regression(arch, formulation; regenerate_data = false)
    ...
    uⁱ(x, y) = ū(x, y) + small_amplitude * exp(-y^2) * cos(8π * x / Lx)
    uhⁱ(x, y) = uⁱ(x, y) * h̄(x, y)

    if formulation isa VectorInvariantFormulation
        set!(model, u = uⁱ, h = h̄)
    else
        set!(model, uh = uhⁱ, h = h̄)   # FJP: this no longer works?
    end
