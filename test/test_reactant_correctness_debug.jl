...
@testset "Reactant correctness" begin
    ...
    if use_mpi
        ...
        reactant_arch = Distributed(reactant_arch)
        # reactant_arch = Distributed(get(ENV, "TEST_ARCHITECTURE", "CPU") == "GPU" ? GPU() : CPU())
    end
    ...
    # debug
    # -------------------
    all_locations = [first(all_locations)]
    all_topologies = [first(all_topologies)]
    raise_modes = [first(raise_modes)]
    # -------------------
