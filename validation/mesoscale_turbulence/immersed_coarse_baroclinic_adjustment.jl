...
adv_closure = IsopycnalSkewSymmetricDiffusivity(; κ_skew, skew_flux_formulation = AdvectiveFormulation())
dif_closure = IsopycnalSkewSymmetricDiffusivity(; κ_skew, skew_flux_formulation = DiffusiveFormulation())
...
function run_simulation(closure, grid)
    ...
    simulation.output_writers[:fields] = JLD2Writer(model, merge(model.tracers, model.diffusivity_fields),
...
vta = FieldTimeSeries(filepath_adv, "v")
wta = FieldTimeSeries(filepath_adv, "w")
...
n  = Observable(1)
ua = @lift interior(uta[$n], 1, :, :)
ba = @lift interior(bta[$n], 1, :, :)
ca = @lift interior(cta[$n], 1, :, :)

# ud = @lift interior(utd[$n], 1, :, :)
# bd = @lift interior(btd[$n], 1, :, :)
# cd = @lift interior(ctd[$n], 1, :, :)
...
contour!(axud, y, z, ba; levels, color=:black, linewidth=2)
cb = Colorbar(fig[2, 3], hm)

hm = heatmap!(axcd, ca, colorrange=(0, 0.5), colormap=:speed)
contour!(axcd, y, z, ba; levels, color=:black, linewidth=2)
