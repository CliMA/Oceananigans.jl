...
# version in `targeted_transport_xy_hydrostatic_flather.jl`.
#
...
stop_time = 1day
ν = 10                        # viscosity [m² s⁻¹]
schemes = (PerturbationAdvection, NormalRadiation, ObliqueRadiation)
...
open_boundary(Scheme, target) = NormalFlowBoundaryCondition(0; scheme = Scheme(; inflow_timescale = Inf, outflow_timescale = Inf,
                                                                                  target_transport = target))

function run_experiment(Scheme, targets)
    boundary_conditions = (u = FieldBoundaryConditions(west = open_boundary(Scheme, targets.west), east = open_boundary(Scheme, targets.east)),
                           v = FieldBoundaryConditions(south = open_boundary(Scheme, targets.south), north = open_boundary(Scheme, targets.north)))
...
for Scheme in schemes
    models = [run_experiment(Scheme, targets) for (name, targets) in experiments]

    println("$Scheme, final transports in units of Q:")
    ...
    Label(fig[0, 1:4], "Nonhydrostatic, $Scheme with target_transport"; fontsize = 18, font = :bold)
    ...
    save("targeted_transport_xy_nonhydrostatic_$Scheme.png", fig)
end
