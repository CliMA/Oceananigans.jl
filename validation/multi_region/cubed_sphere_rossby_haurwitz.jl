...
γ = (n * (3 + n) * ω - 2 * Ω) / ((1 + n) * (2 + n))

ψ_function(θ, ϕ, t) = -R^2 * ω * sin(θ) + R^2 * K * cos(θ)^n * sin(θ) * cos(n*ϕ - γ*t)

u_function(θ, ϕ, t) =  R * ω * cos(θ) + R * K * cos(θ)^(n-1) * (n * sin(θ)^2 - cos(θ)^2) * cos(n*ϕ - γ*t)
v_function(θ, ϕ, t) = -n * K * R * cos(θ)^(n-1) * sin(θ) * sin(n*ϕ - γ*t)

h_function(θ, ϕ, t) = H + R^2/g * (A(θ) + B(θ) * cos(n*ϕ - γ*t) + C(θ) * cos(2(n*ϕ - γ*t)))

ζ_function(θ, ϕ, t) = 2 * ω * sin(θ) - K * sin(θ) * cos(θ)^n * (n^2 + 3n + 2) * cos(n*ϕ - γ*t)
...
=#
η₀(λ, ϕ)    = h_function(rescale²(ϕ), rescale¹(λ), 0)

#=
set!(model, u=u₀, v=v₀, η = η₀)
=#

ψ₀(λ, φ, z) = ψ_function(rescale²(φ), rescale¹(λ), 0)
...
# Redefine ηᵢ as ηᵢ = ηᵢ - ηᵢ_mean.
for region in 1:number_of_regions(grid)
    for j in 1-Hy:Ny+Hy, i in 1-Hx:Nx+Hx, k in Nz+1:Nz+1
        simulation.model.free_surface.η[region][i, j, k] -= ηᵢ_mean
...
function zero_out_halos!(array, Nx, Ny, Hx, Hy, k_min, k_max)
    array[1-Hx:0, :, k_min:k_max]     .= 0
    array[Nx+1:Nx+Hx, :, k_min:k_max] .= 0
    array[:, 1-Hy:0, k_min:k_max]     .= 0
    array[:, Ny+1:Ny+Hy, k_min:k_max] .= 0

    return nothing
end

function compute_exact_solution_and_error!(u_exact, v_exact, ζ_exact, η_exact, simulation_time, u, v, ζ, η, u_error,
                                           v_error, ζ_error, η_error)
    for region in 1:number_of_regions(grid)
        for j in 1:Ny, i in 1:Nx, k in 1:Nz
            λ = λnode(i, j, k, grid[region], Face(), Center(), Center())
            φ = φnode(i, j, k, grid[region], Face(), Center(), Center())
            u_exact[region][i, j, k] = u_function(rescale²(φ), rescale¹(λ), simulation_time)
            u_error[region][i, j, k] = u[region][i, j, k] - u_exact[region][i, j, k]
            λ = λnode(i, j, k, grid[region], Center(), Face(), Center())
            φ = φnode(i, j, k, grid[region], Center(), Face(), Center())
            v_exact[region][i, j, k] = v_function(rescale²(φ), rescale¹(λ), simulation_time)
            v_error[region][i, j, k] = v[region][i, j, k] - v_exact[region][i, j, k]
            λ = λnode(i, j, k, grid[region], Face(), Face(), Center())
            φ = φnode(i, j, k, grid[region], Face(), Face(), Center())
            ζ_exact[region][i, j, k] = ζ_function(rescale²(φ), rescale¹(λ), simulation_time)
            ζ_error[region][i, j, k] = ζ[region][i, j, k] - ζ_exact[region][i, j, k]
        end
        for j in 1:Ny, i in 1:Nx, k in Nz+1:Nz+1
            λ = λnode(i, j, k, grid[region], Center(), Center(), Face())
            φ = φnode(i, j, k, grid[region], Center(), Center(), Face())
            η_exact[region][i, j, k] = h_function(rescale²(φ), rescale¹(λ), simulation_time) - ηᵢ_mean
            η_error[region][i, j, k] = η[region][i, j, k] - η_exact[region][i, j, k]
        end
        zero_out_halos!(u_error[region], Nx, Ny, Hx, Hy, 1, Nz)
        zero_out_halos!(v_error[region], Nx, Ny, Hx, Hy, 1, Nz)
        zero_out_halos!(ζ_error[region], Nx, Ny, Hx, Hy, 1, Nz)
        zero_out_halos!(η_error[region], Nx, Ny, Hx, Hy, Nz+1, Nz+1)
    end
end
...
plot_exact_solution_and_error = false
if plot_initial_field
    ...
    if plot_exact_solution_and_error
        u_exact = XFaceField(grid)
        v_exact = YFaceField(grid)
        ζ_exact = Field{Face, Face, Center}(grid)
        η_exact = CenterField(grid, indices = (:, :, Nz+1:Nz+1))
        u_error = XFaceField(grid)
        v_error = YFaceField(grid)
        ζ_error = Field{Face, Face, Center}(grid)
        η_error = CenterField(grid, indices = (:, :, Nz+1:Nz+1))
        
        simulation_time = 0
        
        compute_exact_solution_and_error!(u_exact, v_exact, ζ_exact, η_exact, simulation_time, uᵢ, vᵢ, ζᵢ, ηᵢ, u_error,
                                          v_error, ζ_error, η_error)
        
        fig = panel_wise_visualization_with_halos(grid, u_exact; k = Nz)
        save("cubed_sphere_rossby_haurwitz_wave_u₀_exact_with_halos.png", fig)
        
        fig = panel_wise_visualization(grid, u_exact; k = Nz)
        save("cubed_sphere_rossby_haurwitz_wave_u₀_exact.png", fig)

        fig = panel_wise_visualization(grid, u_error; k = Nz)
        save("cubed_sphere_rossby_haurwitz_wave_u₀_error.png", fig)
        
        fig = panel_wise_visualization_with_halos(grid, v_exact; k = Nz)
        save("cubed_sphere_rossby_haurwitz_wave_v₀_exact_with_halos.png", fig)
        
        fig = panel_wise_visualization(grid, v_exact; k = Nz)
        save("cubed_sphere_rossby_haurwitz_wave_v₀_exact.png", fig)
        
        fig = panel_wise_visualization(grid, v_error; k = Nz)
        save("cubed_sphere_rossby_haurwitz_wave_v₀_error.png", fig)        
        
        fig = panel_wise_visualization_with_halos(grid, ζ_exact; k = Nz)
        save("cubed_sphere_rossby_haurwitz_wave_ζ₀_exact_with_halos.png", fig)
        
        fig = panel_wise_visualization(grid, ζ_exact; k = Nz)
        save("cubed_sphere_rossby_haurwitz_wave_ζ₀_exact.png", fig)
        
        fig = panel_wise_visualization(grid, ζ_error; k = Nz)
        save("cubed_sphere_rossby_haurwitz_wave_ζ₀_error.png", fig)
    end
end
...
if plot_final_field
    ...
    if plot_exact_solution_and_error
        simulation_time = simulation_time_per_frame * (n_frames - 1)
        
        compute_exact_solution_and_error!(u_exact, v_exact, ζ_exact, η_exact, simulation_time, u_fields[end],
                                          v_fields[end], ζ_fields[end], η_fields[end], u_error, v_error, ζ_error,
                                          η_error)
        
        fig = panel_wise_visualization_with_halos(grid, u_exact; k = Nz)
        save("cubed_sphere_rossby_haurwitz_wave_u_exact_with_halos.png", fig)
        
        fig = panel_wise_visualization(grid, u_exact; k = Nz)
        save("cubed_sphere_rossby_haurwitz_wave_u_exact.png", fig)

        fig = panel_wise_visualization(grid, u_error; k = Nz)
        save("cubed_sphere_rossby_haurwitz_wave_u_error.png", fig)
        
        fig = panel_wise_visualization_with_halos(grid, v_exact; k = Nz)
        save("cubed_sphere_rossby_haurwitz_wave_v_exact_with_halos.png", fig)
        
        fig = panel_wise_visualization(grid, v_exact; k = Nz)
        save("cubed_sphere_rossby_haurwitz_wave_v_exact.png", fig)
        
        fig = panel_wise_visualization(grid, v_error; k = Nz)
        save("cubed_sphere_rossby_haurwitz_wave_v_error.png", fig)        
        
        fig = panel_wise_visualization_with_halos(grid, ζ_exact; k = Nz)
        save("cubed_sphere_rossby_haurwitz_wave_ζ_exact_with_halos.png", fig)
        
        fig = panel_wise_visualization(grid, ζ_exact; k = Nz)
        save("cubed_sphere_rossby_haurwitz_wave_ζ_exact.png", fig)
        
        fig = panel_wise_visualization(grid, ζ_error; k = Nz)
        save("cubed_sphere_rossby_haurwitz_wave_ζ_error.png", fig)
        
        fig = panel_wise_visualization_with_halos(grid, η_exact; k = Nz + 1, ssh = true)
        save("cubed_sphere_rossby_haurwitz_wave_η_exact_with_halos.png", fig)
        
        fig = panel_wise_visualization(grid, η_exact; k = Nz + 1, ssh = true)
        save("cubed_sphere_rossby_haurwitz_wave_η_exact.png", fig)
        
        fig = panel_wise_visualization(grid, η_error; k = Nz + 1, ssh = true)
        save("cubed_sphere_rossby_haurwitz_wave_η_error.png", fig)
    end
end
