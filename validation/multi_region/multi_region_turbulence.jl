...
ϵ(x, y, z)  =  2rand() - 1
u_init = interior(set!(Field(Face, Center, Center), grid), ϵ)
v_init = interior(set!(Field(Face, Center, Center), grid), ϵ)
...
model = HydrostaticFreeSurfaceModel(; grid = mrg, momentum_advection, free_surface,
                                    tracers = (),
                                    buoyancy = nothing,
                                    closure = ScalarDiffusivity(ν=1e-4))

Random.seed!(1234)
set!(model, u=u_init_mrg, v=v_init_mrg)
