...
underlying_grid = RegularRectilinearGrid(size=(256, 64), x=(-5, 15), y=(0, 5), topology=(Periodic, Bounded, Flat))
...
uh_sponge = Relaxation(rate=damping_rate, mask=smoothed_step_mask, target=U)
 h_sponge = Relaxation(rate=damping_rate, mask=smoothed_step_mask, target=U)

model = ShallowWaterModel(grid = grid, gravitational_acceleration=1,
                          forcing = (uh=uh_sponge, h=h_sponge))
...
wizard = TimeStepWizard(cfl=0.5, Δt=0.1*grid.Δx, max_change=1.1, max_Δt=0.5)
