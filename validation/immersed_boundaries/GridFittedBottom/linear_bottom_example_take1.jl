...
ax_u = Axis(fig[1, 1]; title = "u (take 1)", axis_kwargs...)
hm_u = heatmap!(ax_u, xu, zu, uₙ; colormap = :balance)
cn_u = contour!(ax_u, xb, zb, bₙ, levels=30, color="black")
Colorbar(fig[1,2], hm_u, label = "m s⁻¹")

save("u_b_final_take1.png", fig)

fig = Figure(size = (700, 700))

ax_v = Axis(fig[1, 1]; title = "v (take 1)", axis_kwargs...)
hm_v = heatmap!(ax_v, xv, zv, vₙ; colormap = :balance)
cn_v = contour!(ax_v, xb, zb, bₙ, levels=30, color="black")
