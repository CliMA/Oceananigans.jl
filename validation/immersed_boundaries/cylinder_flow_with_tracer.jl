...
const R = 1.0          # radius
const Re = 40.0        # Reynolds number, Re < 40 --> Steady State
...
"""
function run_cylinder_steadystate(; output_time_interval = 1, stop_time = 100, arch = CPU(), Nh = 250, ν = 1/20,
                                    advection = UpwindBiasedFifthOrder(), radius = R)


    inside_cylinder(x, y, z) = (x^2 + y^2) <= radius # immersed solid

    underlying_grid = RectilinearGrid(arch, size=(Nh, Int(3Nh/2), 1), halo=(3, 3, 3),
    ...
    Δt = 0.07 * xspacings(underlying_grid, Face(), Center(), Center())
    simulation = Simulation(immersed_model, Δt=Δt, stop_time=stop_time)
...
function visualize_cylinder_steadystate(experiment_name)
    ...
    anim = @animate for (i, t) in enumerate(p_timeseries.times)
        ...
        u_plot = heatmap(xu, yu, clamp.(uᵢ -1, 1)'; color = :balance, size = (500,700),
                         guidefontsize = 14, titlefont=14, top_margin = -10.0mm, bottom_margin = -10.0mm,
                         legendfont = 10, tickfont = 8, kwargs...)
        plot!(circle_shape(0, 0, 1), seriestype=[:shape,], linecolor=:black, legend=false, fillalpha=0)
        
        v_plot = heatmap(xv, yv, clamp.(vᵢ, -1, 1)'; color = :balance, size = (500,700),
                         guidefontsize = 14, titlefont=14, top_margin = -10.0mm, bottom_margin = -10.0mm,
                         legendfont = 10, tickfont = 8, kwargs...)
        plot!(circle_shape(0, 0, 1), seriestype=[:shape,], linecolor=:black, legend=false, fillalpha=0)
        
        p_plot = heatmap(xp, yp, clamp.(pᵢ, -1, 1)'; color = :balance, size = (500,700),
                         guidefontsize = 14, titlefont=14, top_margin = -10.0mm, bottom_margin = -10.0mm,
                         legendfont = 10, tickfont = 8, kwargs...)
        plot!(circle_shape(0, 0, 1), seriestype=[:shape,], linecolor=:black, legend=false, fillalpha=0)

        m_plot = heatmap(xm, ym, clamp.(mᵢ, -1, 1)'; color = :thermal, size = (500,700),
                         guidefontsize = 14, titlefont=14, top_margin = -10.0mm, bottom_margin = -10.0mm,
                         legendfont = 10, tickfont = 8, kwargs...)
        plot!(circle_shape(0, 0, 1), seriestype=[:shape,], linecolor=:black, legend=false, fillalpha=0)
    ...
    return mp4(anim, "contours_" * experiment_name * ".mp4", fps = 8)
end
...
function analyze_cylinder_steadystate(experiment_name)
    ...
    "distance for solid cylinder"
    @inline dist_cylinder(v) = sqrt(v[1]^2 + v[2]^2) - R

    # find the sfc normal vector
    normalDist = x-> ForwardDiff.gradient(dist_cylinder, x)

    "create the projection matrix into surface normal and tangential"
    function projection_matrix(N)
        ...
        return transpose(hcat(v1,v2,N)) # creating a matrix of all 3 vectors
    end
    
    "interpolate velocity vector at vector location xvec"
    ...
    "Output normal and tangential velocity, pressure, and friction coefficient
    ...
    function surface_values(M, U, V, W, Pnon, grid, eps, ν)
        ...
        θ     = LinRange(-π/2, 3π/2-2π/M, M)
        x₀    = hcat(R * cos.(θ), R * sin.(θ), 0.5 * ones(M,1))
        Vⁿ    = zeros(M, 1)
        Vᵗ¹   = zeros(M, 1)
        p_sfc = zeros(M, 1)
        dVt   = zeros(M, 1)
    ...
    Vnt = plot(rad2deg.(θ), Vⁿ, xlabel="θ", ylabel="Velocity", seriestype=:scatter,
    ...
    plot!(rad2deg.(θ), Vᵗ, label= "Vᵗ", seriestype=:scatter, markersize = 3)

    @info "Plotting Pressure Coefficient..." 

    Pcoef = plot(rad2deg.(θ), 2 .* p_sfc, xlabel="θ", ylabel= "Cp", seriestype=:scatter,
    ...
    Fcoef = plot(rad2deg.(θ), Cf, xlabel="θ", ylabel= "Cf", seriestype=:scatter,
                 guidefontsize = 14, titlefont=14,legendfont = 10, tickfont = 8,
                 xticks = [-90, 0, 90, 180, 275], xlims = (-90, 275), markersize = 3,
                 legend = false, size = (700, 500))
