...
reltol = abstol = 1e-7
pcg_local = ConjugateGradientPoissonSolver(local_grid, maxiter=1000; reltol, abstol, preconditioner=nothing)
pcg_global = ConjugateGradientPoissonSolver(global_grid, maxiter=1000; reltol, abstol, preconditioner=nothing)
...
@info "PCG iteration $(pcg_global.conjugate_gradient_solver.iteration), time $(t_pcg.time)"
...
@info "PCG iteration $(pcg_local.conjugate_gradient_solver.iteration), time $(t_pcg.time)"

xpcg_reconstruct = reconstruct_global_field(xpcg_local)

@info arch.local_rank all(interior(xpcg_reconstruct) .≈ interior(xpcg_global))
