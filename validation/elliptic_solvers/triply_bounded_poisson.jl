...
# x, y = grid.xᶜᵃᵃ, grid.yᵃᶜᵃ

using GLMakie

fig = Figure()
ax1 = Axis(fig[1, 1])
heatmap!(ax1, x[1:Nx], y[1:Ny], grid.immersed_boundary.bottom_height[1:Nx, 1:Ny])
current_figure()

# Select RHS
r = CenterField(grid)
r₀(x, y, z) = (x^2 + y^2) < 1 ? 1 : 0 # exp(-x^2 - y^2 - z^2)
...
ax2 = Axis(fig[1, 2])
heatmap!(ax2, x[1:Nx], y[1:Ny], interior(r)[:, :, z_slice])
current_figure()
...
∇²φ_mg = CenterField(grid)
compute_∇²!(∇²φ_mg, φ_mg, arch, grid.underlying_grid)

∇²φ_cg = CenterField(grid)
compute_∇²!(∇²φ_cg, φ_cg, arch, grid.underlying_grid)

ax_mg = Axis(fig[2, 2])
heatmap!(ax_mg, x[1:Nx], y[1:Ny], interior(φ_mg)[:, :, z_slice])
current_figure()

ax_lapcg = Axis(fig[3, 1])
heatmap!(ax_lapcg, x[1:Nx], y[1:Ny], interior(∇²φ_cg)[:, :, z_slice])
current_figure()

ax_lapmg = Axis(fig[3, 2])
heatmap!(ax_lapmg, x[1:Nx], y[1:Ny], interior(∇²φ_mg)[:, :, z_slice])
current_figure()
