...
"""
function MultigridPreconditioner(linear_opearation::Function, template_field; maxiter=1)
    mgs = MultigridSolver(linear_opearation; template_field, maxiter, amg_algorithm = RugeStubenAMG())
...
@time cgmg_solver = PreconditionedConjugateGradientSolver(compute_∇²!, template_field=r, reltol=eps(eltype(grid)), preconditioner = mgp)
