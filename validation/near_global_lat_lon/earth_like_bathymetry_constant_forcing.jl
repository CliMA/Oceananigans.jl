...
horizontal_diffusivity = HorizontallyCurvilinearAnisotropicDiffusivity(νh=νh₂, νz=νz, κh=κh, κz=κz)
...
T_surface_relaxation_bc = FluxBoundaryCondition(surface_temperature_relaxation,
                                                discrete_form = true,
                                                parameters = (λ = 1/3days, T★ = target_sea_surface_temperature))
...
model = HydrostaticFreeSurfaceModel(grid = grid,
                                    ...
                                    closure = nothing, # (horizontal_diffusivity, convective_adjustment),
                                    tracers = (:T, :S))
...
simulation = Simulation(model, Δt = Δt, stop_iteration = 10, iteration_interval = 1, progress = progress)
