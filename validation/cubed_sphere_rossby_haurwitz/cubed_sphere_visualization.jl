...
function specify_colorrange(grid, φ, use_symmetric_colorrange = true, ssh = false)
    ...
    for region in 1:6
        φ_array[:, :, :, region] = φ.data[region][1:Nx, 1:Ny, φ_array_vertical_dimension_limits]
    end
...
function specify_colorrange_timeseries(grid, φ_series, use_symmetric_colorrange = true, ssh = false)
    ...
    for i in 1:n
        for region in 1:6
            φ_series_array[:, :, :, region, i] = φ_series[i].data[region][1:Nx, 1:Ny, 
...
function panel_wise_visualization_with_halos(grid, field, k = 1, use_symmetric_colorrange = true, ssh = false)
    ...
    ax_1 = Axis(fig[3, 1]; title = "Panel 1", axis_kwargs...)
    hm_1 = heatmap!(ax_1, parent(field.data[1][:, :, k]); colorrange, colormap)
    Colorbar(fig[3, 2], hm_1)

    ax_2 = Axis(fig[3, 3]; title = "Panel 2", axis_kwargs...)
    hm_2 = heatmap!(ax_2, parent(field.data[2][:, :, k]); colorrange, colormap)
    Colorbar(fig[3, 4], hm_2)

    ax_3 = Axis(fig[2, 3]; title = "Panel 3", axis_kwargs...)
    hm_3 = heatmap!(ax_3, parent(field.data[3][:, :, k]); colorrange, colormap)
    Colorbar(fig[2, 4], hm_3)

    ax_4 = Axis(fig[2, 5]; title = "Panel 4", axis_kwargs...)
    hm_4 = heatmap!(ax_4, parent(field.data[4][:, :, k]); colorrange, colormap)
    Colorbar(fig[2, 6], hm_4)

    ax_5 = Axis(fig[1, 5]; title = "Panel 5", axis_kwargs...)
    hm_5 = heatmap!(ax_5, parent(field.data[5][:, :, k]); colorrange, colormap)
    Colorbar(fig[1, 6], hm_5)

    ax_6 = Axis(fig[1, 7]; title = "Panel 6", axis_kwargs...)
    hm_6 = heatmap!(ax_6, parent(field.data[6][:, :, k]); colorrange, colormap)
    Colorbar(fig[1, 8], hm_6)
...
function panel_wise_visualization(grid, field, k = 1, use_symmetric_colorrange = true, ssh = false)
    ...
    ax_1 = Axis(fig[3, 1]; title = "Panel 1", axis_kwargs...)
    hm_1 = heatmap!(ax_1, parent(field.data[1][1:Nx, 1:Ny, k]); colorrange, colormap)
    Colorbar(fig[3, 2], hm_1)

    ax_2 = Axis(fig[3, 3]; title = "Panel 2", axis_kwargs...)
    hm_2 = heatmap!(ax_2, parent(field.data[2][1:Nx, 1:Ny, k]); colorrange, colormap)
    Colorbar(fig[3, 4], hm_2)

    ax_3 = Axis(fig[2, 3]; title = "Panel 3", axis_kwargs...)
    hm_3 = heatmap!(ax_3, parent(field.data[3][1:Nx, 1:Ny, k]); colorrange, colormap)
    Colorbar(fig[2, 4], hm_3)

    ax_4 = Axis(fig[2, 5]; title = "Panel 4", axis_kwargs...)
    hm_4 = heatmap!(ax_4, parent(field.data[4][1:Nx, 1:Ny, k]); colorrange, colormap)
    Colorbar(fig[2, 6], hm_4)

    ax_5 = Axis(fig[1, 5]; title = "Panel 5", axis_kwargs...)
    hm_5 = heatmap!(ax_5, parent(field.data[5][1:Nx, 1:Ny, k]); colorrange, colormap)
    Colorbar(fig[1, 6], hm_5)

    ax_6 = Axis(fig[1, 7]; title = "Panel 6", axis_kwargs...)
    hm_6 = heatmap!(ax_6, parent(field.data[6][1:Nx, 1:Ny, k]); colorrange, colormap)
    Colorbar(fig[1, 8], hm_6)
...
function create_panel_wise_visualization_animation_with_halos(grid, φ_series, start_index, use_symmetric_colorrange, 
    ...
    ax_1 = Axis(fig[3, 1]; title = "Panel 1", axis_kwargs...)
    hm_1 = heatmap!(ax_1, parent(φ_series[start_index].data[1][:, :, k]); colorrange, colormap)
    Colorbar(fig[3, 2], hm_1)

    ax_2 = Axis(fig[3, 3]; title = "Panel 2", axis_kwargs...)
    hm_2 = heatmap!(ax_2, parent(φ_series[start_index].data[2][:, :, k]); colorrange, colormap)
    Colorbar(fig[3, 4], hm_2)

    ax_3 = Axis(fig[2, 3]; title = "Panel 3", axis_kwargs...)
    hm_3 = heatmap!(ax_3, parent(φ_series[start_index].data[3][:, :, k]); colorrange, colormap)
    Colorbar(fig[2, 4], hm_3)

    ax_4 = Axis(fig[2, 5]; title = "Panel 4", axis_kwargs...)
    hm_4 = heatmap!(ax_4, parent(φ_series[start_index].data[4][:, :, k]); colorrange, colormap)
    Colorbar(fig[2, 6], hm_4)

    ax_5 = Axis(fig[1, 5]; title = "Panel 5", axis_kwargs...)
    hm_5 = heatmap!(ax_5, parent(φ_series[start_index].data[5][:, :, k]); colorrange, colormap)
    Colorbar(fig[1, 6], hm_5)

    ax_6 = Axis(fig[1, 7]; title = "Panel 6", axis_kwargs...)
    hm_6 = heatmap!(ax_6, parent(φ_series[start_index].data[6][:, :, k]); colorrange, colormap)
    Colorbar(fig[1, 8], hm_6)

    # Use an on block to reactively update the visualization.
    on(n) do index
        hm_1 = heatmap!(ax_1, parent(φ_series[index].data[1][:, :, k]); colorrange, colormap)
        Colorbar(fig[3, 2], hm_1)

        hm_2 = heatmap!(ax_2, parent(φ_series[index].data[2][:, :, k]); colorrange, colormap)
        Colorbar(fig[3, 4], hm_2)

        hm_3 = heatmap!(ax_3, parent(φ_series[index].data[3][:, :, k]); colorrange, colormap)
        Colorbar(fig[2, 4], hm_3)

        hm_4 = heatmap!(ax_4, parent(φ_series[index].data[4][:, :, k]); colorrange, colormap)
        Colorbar(fig[2, 6], hm_4)

        hm_5 = heatmap!(ax_5, parent(φ_series[index].data[5][:, :, k]); colorrange, colormap)
        Colorbar(fig[1, 6], hm_5)

        hm_6 = heatmap!(ax_6, parent(φ_series[index].data[6][:, :, k]); colorrange, colormap)
        Colorbar(fig[1, 8], hm_6)
    end
