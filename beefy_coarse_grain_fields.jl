...
levels = [3, collect(9:14)...]
timekeys = 1126:length(ηkeys)

Ns = reverse([2^i for i in 3:8])
for N in Ns
    M = 256 ÷ N
    @info "saving eta"
    ...
    hfile = h5open(analysis_directory * "training_baroclinic_double_gyre_$(M)_$casevar.hdf5", "w")
    ...
    hfile = h5open(analysis_directory * "training_baroclinic_double_gyre_$(M)_$casevar.hdf5", "r+")
