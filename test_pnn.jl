...
function solve_poisson_3d_pnn!(g::RegularCartesianGrid, f::CellField, ϕ::CellField)
    kx² = zeros(g.Nx, 1)
    ky² = zeros(g.Ny, 1)
    kz² = zeros(g.Nz, 1)
    ...
    @. ϕ.data = real(ϕ.data) / (2g.Ny*2g.Nz)

    FFTW.r2r!(ϕ.data, FFTW.REDFT01, [2, 3])
