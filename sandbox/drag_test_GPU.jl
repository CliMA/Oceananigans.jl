...
aux_grid = RectilinearGrid(arch; size = Nz,
                           z = (0, 1),
                           halo = 3,
...
const Cᴰ = drag_C(aux_grid.Δzᵃᵃᶜ)

@inline τʸˣ_BC_west(x, y, t, u, v, w) = +Cᴰ * v * (v^2 + w^2)^0.5
@inline τᶻˣ_BC_west(x, y, t, u, v, w) = +Cᴰ * w * (v^2 + w^2)^0.5

@inline τˣʸ_BC_south(x, y, t, u, v, w) = Cᴰ * u * (u^2 + w^2)^0.5
@inline τᶻʸ_BC_south(x, y, t, u, v, w) = Cᴰ * w * (u^2 + w^2)^0.5

@inline τˣᶻ_BC_bottom(x, y, t, u, v, w) = Cᴰ * u * (u^2 + v^2)^0.5
@inline τʸᶻ_BC_bottom(x, y, t, u, v, w) = Cᴰ * v * (u^2 + v^2)^0.5

@inline τʸˣ_BC_east(x, y, t, u, v, w) = -Cᴰ * v * (v^2 + w^2)^0.5
@inline τᶻˣ_BC_east(x, y, t, u, v, w) = -Cᴰ * w * (v^2 + w^2)^0.5
...
for i in 1:3
    ...
    global grid = RectilinearGrid(arch; size = sizes,
                                  kwargs...,
                                  halo = (3,3,3),
                                  topology = topo)
    @show grid
    ...
    global grid_ext = RectilinearGrid(arch; size = sizes_ext,
                                      kwargs_ext...,
                                      halo = (3,3,3),
                                      topology = topo)
    @show grid_ext
    ...
    global control_drag_model = NonhydrostaticModel(grid = grid; boundary_conditions = boundary_conditions, kwargs_model...)
    global immersed_model = NonhydrostaticModel(grid = immersed_grid; kwargs_model...)
