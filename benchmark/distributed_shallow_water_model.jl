...
plt = plot(rank_num, run_times, lw=4, xaxis=:log2, legend=:none,
          xlabel=label, ylabel="Times (ms)", title=string(title, " ", dis_type, " scaling shallow water times"))
display(plt)
savefig(plt, string(title, "_", dis_type, "_shallow_water_times.png"))


plt2 = plot(rank_num, eff_ratio, lw=4, xaxis=:log2, legend=:none, ylims=(0,1.1),
            xlabel=label, ylabel="Efficiency", title=string(title, " ", dis_type, " scaling shallow water efficiency"))
display(plt2)
savefig(plt2, string(title, "_", dis_type, "_shallow_water_efficiency.png"))

# Summarize benchmarks

df = benchmarks_dataframe(suite)
sort!(df, Symbol(label))
benchmarks_pretty_table(df, title=string(title, " ", dis_type," scaling shallow water times"))
...
benchmarks_pretty_table(df_Δ, title=string(title, " ", dis_type," scaling shallow water efficiency"))
