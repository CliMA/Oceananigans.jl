...
function NNFluxClosure(arch)
    ...
    nn_path = "./NDE_FC_Qb_Ri_nof_BBLkappazonelast55_trainFC19new_scalingtrain59new_2layer_256_relu_10seed_2Pr_model_temp.jld2"
...
function DiffusivityFields(grid, tracer_names, bcs, ::NNFluxClosure)
    ...
    wrk_in = zeros(13, Nx_in, Ny_in, 10)
    wrk_in = on_architecture(arch, wrk_in)

    wrk_wT = zeros(Nx_in, Ny_in, 10)
    wrk_wS = zeros(Nx_in, Ny_in, 10)
...
@kernel function _populate_input!(input, first_index, last_index, grid, closure::NNFluxClosure, tracers, velocities, buoyancy, top_tracer_bcs, Ri, clock)
    ...
    @inbounds input[1, i, j, k] = Riᵢ₋₁ = ifelse(quiescent, 0, atan(Ri[i, j, k_tracer-1]))
    @inbounds input[2, i, j, k] = Riᵢ   = ifelse(quiescent, 0, atan(Ri[i, j, k_tracer]))
    @inbounds input[3, i, j, k] = Riᵢ₊₁ = ifelse(quiescent, 0, atan(Ri[i, j, k_tracer+1]))

    @inbounds input[4, i, j, k] = ∂Tᵢ₋₁ = scaling.∂T∂z(ifelse(quiescent, 0, ∂zᶜᶜᶠ(i, j, k_tracer-1, grid, T)))
    @inbounds input[5, i, j, k] = ∂Tᵢ   = scaling.∂T∂z(ifelse(quiescent, 0, ∂zᶜᶜᶠ(i, j, k_tracer,   grid, T)))
    @inbounds input[6, i, j, k] = ∂Tᵢ₊₁ = scaling.∂T∂z(ifelse(quiescent, 0, ∂zᶜᶜᶠ(i, j, k_tracer+1, grid, T)))

    @inbounds input[7, i, j, k] = ∂Sᵢ₋₁ = scaling.∂S∂z(ifelse(quiescent, 0, ∂zᶜᶜᶠ(i, j, k_tracer-1, grid, S)))
    @inbounds input[8, i, j, k] = ∂Sᵢ   = scaling.∂S∂z(ifelse(quiescent, 0, ∂zᶜᶜᶠ(i, j, k_tracer,   grid, S)))
    @inbounds input[9, i, j, k] = ∂Sᵢ₊₁ = scaling.∂S∂z(ifelse(quiescent, 0, ∂zᶜᶜᶠ(i, j, k_tracer+1, grid, S)))

    @inbounds input[10, i, j, k] = ∂σᵢ₋₁ = scaling.∂ρ∂z(ifelse(quiescent, 0, -ρ₀ * ∂z_b(i, j, k_tracer-1, grid, buoyancy, tracers) / g))
    @inbounds input[11, i, j, k] = ∂σᵢ   = scaling.∂ρ∂z(ifelse(quiescent, 0, -ρ₀ * ∂z_b(i, j, k_tracer,   grid, buoyancy, tracers) / g))
    @inbounds input[12, i, j, k] = ∂σᵢ₊₁ = scaling.∂ρ∂z(ifelse(quiescent, 0, -ρ₀ * ∂z_b(i, j, k_tracer+1, grid, buoyancy, tracers) / g))

    @inbounds input[13, i, j, k] = Jᵇ = scaling.wb(top_buoyancy_flux(i, j, grid, buoyancy, top_tracer_bcs, clock, merge(velocities, tracers)))
...
@kernel function _find_NN_active_region!(κᶜ, grid, κ₀, first_index, last_index)
    ...
    top_index = grid.Nz + 1
    grid_point_above_kappa = 5
    grid_point_below_kappa = 5

    # Find the last index of the background κᶜ
    kloc = 1
    @inbounds for k in 2:grid.Nz
        kloc = ifelse(κᶜ[i, j, k] ≈ κ₀, k, kloc)
    end

    nonbackground_κ_index = kloc + 1

    @inbounds last_index[i, j, 1] = ifelse(nonbackground_κ_index == top_index, top_index - 1, clamp_k_interior(kloc + grid_point_above_kappa, grid))
    @inbounds first_index[i, j, 1] = ifelse(nonbackground_κ_index == top_index, top_index, clamp_k_interior(kloc - grid_point_below_kappa + 1, grid))
end
